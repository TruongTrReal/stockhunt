...
def main():
    ...
    # An empty result is a legitimate outcome, not an error: it means nothing
    # cleared the multiplicity bar. pd.DataFrame([]) has no columns at all, so
    # the column-selected prints below would raise KeyError rather than print
    # an empty table - report and stop instead.
    if df.empty:
        print(f"Nothing clears the bar. With ~173,732 candidates searched, the expected "
              f"max |z| under pure noise is {Z_BAR:.2f}, and no candidate's Jobson-Korkie/"
              f"Memmel z reaches it either as traded or with an extra bar of lag.\n"
              f"An empty {OUT} is the honest result.")
        return
