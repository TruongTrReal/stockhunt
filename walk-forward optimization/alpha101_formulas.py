"""The 101 formulaic alphas of Kakushadze (2016), verbatim.
...
FORMULAS: dict[int, str] = {
    ...
    7: (
        "((adv20 < volume) ? ((-1 * ts_rank(abs(delta(close, 7)), 60)) * sign(delta(close, "
        "7))) : (-1 * 1))"
    ),
    8: (
        "(-1 * rank(((sum(open, 5) * sum(returns, 5)) - delay((sum(open, 5) * sum(returns, "
        "5)), 10))))"
    ),
    9: (
        "((0 < ts_min(delta(close, 1), 5)) ? delta(close, 1) : ((ts_max(delta(close, 1), 5) < "
        "0) ? delta(close, 1) : (-1 * delta(close, 1))))"
    ),
    10: (
        "rank(((0 < ts_min(delta(close, 1), 4)) ? delta(close, 1) : ((ts_max(delta(close, 1), "
        "4) < 0) ? delta(close, 1) : (-1 * delta(close, 1)))))"
    ),
    11: (
    ...
    18: (
        "(-1 * rank(((stddev(abs((close - open)), 5) + (close - open)) + correlation(close, "
        "open, 10))))"
    ),
    19: (
    ...
    21: (
        ...
        "(((sum(close, 2) / 2) < ((sum(close, 8) / 8) - stddev(close, 8))) ? 1 : (((1 < "
    ...
    24: (
        ...
        "((delta((sum(close, 100) / 100), 100) / delay(close, 100)) == 0.05)) ? (-1 * (close "
    ...
    30: (
        ...
        "delay(close, 2)))) + sign((delay(close, 2) - delay(close, 3)))))) * sum(volume, 5)) "
        "/ sum(volume, 20))"
    ),
    31: (
    ...
    32: (
        "(scale(((sum(close, 7) / 7) - close)) + (20 * scale(correlation(vwap, delay(close, "
        "5), 230))))"
    ),
    33: "rank((-1 * ((1 - (open / close))^1)))",
    34: (
        "rank(((1 - rank((stddev(returns, 2) / stddev(returns, 5)))) + (1 - rank(delta(close, "
        "1)))))"
    ),
    35: (
    ...
    36: (
        "(((((2.21 * rank(correlation((close - open), delay(volume, 1), 15))) + (0.7 * "
        "rank((open - close)))) + (0.73 * rank(Ts_Rank(delay((-1 * returns), 6), 5)))) + "
        "rank(abs(correlation(vwap, adv20, 6)))) + (0.6 * rank((((sum(close, 200) / 200) - "
        "open) * (close - open)))))"
    ),
    ...
    46: (
        "((0.25 < (((delay(close, 20) - delay(close, 10)) / 10) - ((delay(close, 10) - close) "
        "/ 10))) ? (-1 * 1) : (((((delay(close, 20) - delay(close, 10)) / 10) - "
        "((delay(close, 10) - close) / 10)) < 0) ? 1 : ((-1 * 1) * (close - delay(close, "
        "1)))))"
    ),
    47: (
    ...
    55: (
        "(-1 * correlation(rank(((close - ts_min(low, 12)) / (ts_max(high, 12) - ts_min(low, "
        "12)))), rank(volume), 6))"
    ),
    56: (
        "(0 - (1 * (rank((sum(returns, 10) / sum(sum(returns, 2), 3))) * rank((returns * "
        "cap)))))"
    ),
    ...
    59: (
        "(-1 * Ts_Rank(decay_linear(correlation(IndNeutralize(((vwap * 0.728317) + (vwap * (1 "
        "- 0.728317))), IndClass.industry), volume, 4.25197), 16.2289), 8.19648))"
    ),
    60: (
    ...
    63: (
        "((rank(decay_linear(delta(IndNeutralize(close, IndClass.industry), 2.25164), "
        "8.22237)) - rank(decay_linear(correlation(((vwap * 0.318108) + (open * (1 - "
        "0.318108))), sum(adv180, 37.2467), 13.557), 12.2883))) * -1)"
    ),
    64: (
    ...
    65: (
        "((rank(correlation(((open * 0.00817205) + (vwap * (1 - 0.00817205))), sum(adv60, "
        "8.6911), 6.40374)) < rank((open - ts_min(open, 13.635)))) * -1)"
    ),
    66: (
        "((rank(decay_linear(delta(vwap, 3.51013), 7.23052)) + Ts_Rank(decay_linear(((((low * "
        "0.96633) + (low * (1 - 0.96633))) - vwap) / (open - ((high + low) / 2))), 11.4157), "
        "6.72611)) * -1)"
    ),
    67: (
    ...
    69: (
        ...
        "4.79344))^Ts_Rank(correlation(((close * 0.490655) + (vwap * (1 - 0.490655))), adv20, "
        "4.92416), 9.0615)) * -1)"
    ),
    70: (
    ...
    80: (
        ...
        "IndClass.industry), 4.04545)))^Ts_Rank(correlation(high, adv10, 5.11456), 5.53756)) "
        "* -1)"
    ),
    81: (
    ...
    82: (
        ...
        "Ts_Rank(decay_linear(correlation(IndNeutralize(volume, IndClass.sector), ((open * "
        "0.634196) + (open * (1 - 0.634196))), 17.4842), 6.92131), 13.4283)) * -1)"
    ),
    83: (
    ...
    85: (
        ...
        "9.61331))^rank(correlation(Ts_Rank(((high + low) / 2), 3.70596), Ts_Rank(volume, "
        "10.1595), 7.11408)))"
    ),
    86: (
        "((Ts_Rank(correlation(close, sum(adv20, 14.7444), 6.00049), 20.4195) < rank(((open + "
        "close) - (vwap + open)))) * -1)"
    ),
    87: (
    ...
    89: (
        ...
        "adv10, 6.94279), 5.51607), 3.79744) - Ts_Rank(decay_linear(delta(IndNeutralize(vwap, "
    ...
    93: (
        ...
        "17.4193), 19.848), 7.54455) / rank(decay_linear(delta(((close * 0.524434) + (vwap * "
        "(1 - 0.524434))), 2.77377), 16.2664)))"
    ),
    94: (
    ...
    97: (
        "((rank(decay_linear(delta(IndNeutralize(((low * 0.721001) + (vwap * (1 - "
        "0.721001))), IndClass.industry), 3.3705), 20.4523)) - "
        "Ts_Rank(decay_linear(Ts_Rank(correlation(Ts_Rank(low, 7.87871), Ts_Rank(adv60, "
        "17.255), 4.97547), 18.5925), 15.7152), 6.71659)) * -1)"
    ),
    98: (
    ...
    100: (
        ...
        "scale(indneutralize((correlation(close, rank(adv20), 5) - rank(ts_argmin(close, "
