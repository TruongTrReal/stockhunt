...
schema_vocab.py  stage 1j: the SEMANTIC VOCABULARY for schema-plan mining -- a factor
   |             is a plan (event, context, qualities, direction, output, window)
   |             before it is an expression. Fixed per run; extend it BETWEEN runs
schema_translate.py  plan -> expression IN THE ALPHA101 GRAMMAR, so parsing, panel
   |             evaluation and the truncation gate are inherited, not rewritten.
   |             Offline template mode is the default; --llm adds Claude realizations
   |             via TWO backends: --llm-backend cli (default -- headless `claude -p`
   |             on the existing Claude Code login, no API key) or api (Anthropic
   |             API, needs ANTHROPIC_API_KEY). The LLM only ever translates a plan;
   |             it never sees a backtest number and never picks what to search
schema_miner.py  the search loop: novelty explore -> ridge-surrogate exploit -> one-
   |             component mutation. Rewards are rank-IC on the IN-SAMPLE span only
   |             (default: the day before the sheet's oos_start); every realized
   |             expression is appended to data/reference/trials.csv BEFORE scoring.
   |             State + audit trail: results/schema_buffer_<class>_<tf>.csv.
   |             IC here is a search signal, NEVER a verdict -- survivors still owe
   |             the full gauntlet out of sample
test_schema_miner.py  the gate for it. __main__, exits nonzero, synthetic bars with a
   |             planted cross-sectional reversal the reward frame must find. ~30s
schema_books.py  the bridge to the gauntlet: exported survivors -> quantile books over
   |             the FULL panel, cached like alpha101's, read back one column at a
   |             time by riskmatch_wf, where `schema_*` is a fifth population in
   |             resolve_position. Pass the miner's buffer row count as --n-trials
...
python riskmatch_wf.py --tf 1d --oos-start 2010-01-01 --rules ...   # first scored
                                     # bar; for rules SELECTED on a wide --is-end.
                                     # Always partial, never --promote. The clamp is
                                     # _clamp_start and it feeds run_sheet AND
                                     # _init_worker -- a span flag that misses the
                                     # worker path scores the default span silently
...
python schema_miner.py --dry-run     # stage 1j: audit the plan space, no data
python schema_miner.py --class us_stocks --tf 1d --rounds 30 --batch 16   # mine
python schema_miner.py --class us_stocks --tf 1d --resume --rounds 10     # continue
python schema_miner.py --class us_stocks --tf 1d --export  # correlation-filtered top
python test_schema_miner.py          # the stage-1j gate. ~30s, run after any edit
python schema_books.py --class us_stocks --tf 1d           # survivors -> books
python riskmatch_wf.py --class us_stocks --tf 1d --n-trials <buffer rows> --rules $(python schema_books.py --class us_stocks --tf 1d --names)
