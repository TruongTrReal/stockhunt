(function () {
  ...
  var TIMEFRAME_TLDR = {
    "1D": "Daily bars, Jan 2015 to Jul 2026 (2,911 bars per stock). Positions can be held for weeks, months or years - no forced exit anywhere. This is the cleanest of the three: the longest history, the lowest turnover, and the timeframe where transaction costs matter least. If an edge exists anywhere in this study it should be most visible here.",
    "1H": "Hourly bars, Jan 2020 to Jul 2026 (~11,400 bars per stock). Positions can be held across days or weeks - there is no forced exit at the end of a session, just finer-grained entries and exits than daily bars would allow. 20 largest US stocks, costs charged at 5bps per unit of position change.",
    "5m": "5-minute bars, Jan 2023 to Jul 2026 (~69,700 bars per stock). Every rule IS force-closed at the last bar of each session - zero overnight exposure, re-entered fresh the next morning if it still signals. Buy-&-hold is NOT flattened, because stripping its overnight drift would make it a different strategy. Costs bite hardest here: a 5-minute rule can turn over hundreds of times a year.",
  };
  ...
  function renderMeta() {
    ...
    var chips = [
      ...
      ["Candidates", m.n_singles + " singles + " + m.n_combos.toLocaleString() + " combos"],
      ["Cost charged", m.cost_bps + "bps per unit traded"],
      ["Beat buy &amp; hold", m.n_beat_buyhold + " of " + m.n_candidates.toLocaleString()],
      ["Noise bar", "excess Sharpe " + fmtNum(m.noise_bar_excess_sharpe, 2) +
        " needed; best is " + fmtNum(m.best_excess_sharpe, 2)],
    ];
    ...
    document.getElementById("eyebrow-count").textContent =
      m.n_singles + " singles \u00b7 " + m.n_combos.toLocaleString() + " combos \u00b7 " +
      m.n_tickers + " stocks \u00b7 net of " + m.cost_bps + "bps \u00b7 " + currentTimeframe;
    document.getElementById("leaderboard-title").textContent =
      "Full leaderboard \u2014 all " + m.n_indicators.toLocaleString() +
      " candidates (" + currentTimeframe + ")";
  ...
  var COLUMNS = [
    ...
    // The column that actually settles the question. Raw Sharpe in a rising market
    // mostly measures how long-biased a rule is; excess over buy-&-hold does not.
    { key: "avg_excess_sharpe", label: "vs B&H", fmt: function (v) { return fmtNum(v, 3); }, colorize: true },
    // IR is the Sharpe of the (strategy - buy&hold) difference series, so unlike
    // "vs B&H" it keeps the correlation to the benchmark instead of discarding it.
    { key: "avg_ir", label: "IR", fmt: function (v) { return fmtNum(v, 2); }, colorize: true },
    // Share of the 20 stocks where the IR is positive. A strong IR with a low hit rate
    // is an edge carried by one or two names, not an edge.
    { key: "ir_hit_rate", label: "IR Hit", fmt: function (v) { return fmtPct(v, 0); } },
    // t = IR * sqrt(years). The significance of an edge is its IR scaled by how long it
    // has been observed - which is why 8 years caps even a good IR of 0.5 at t = 1.41.
    { key: "ir_t", label: "t", fmt: function (v) { return fmtNum(v, 2); }, colorize: true },
    // Cost level at which the IR reaches zero. 0 means it never had headroom to give:
    // the rule is behind buy-&-hold before a cent of cost is charged.
    { key: "breakeven_bps", label: "BE bps", fmt: function (v) { return fmtNum(v, 1); } },
