"""Fit an AR(3) by Yule-Walker on centred prices and take its one-step forecast."""

from __future__ import annotations

import numpy as np

from strategies._forecast import (autocorr, expose, rmean, shift, win)

RULE = 'Fit an AR(3) by Yule-Walker on centred prices and take its one-step forecast.'
SOURCE = "megacellar '164 Trading Strategy Browser' (megacellar-browser.onrender.com), 'RollingAR3'"
FAMILY = 'reversion'
DRAFT = False
ANCHOR = False
CLASSES = None
...
What the source published
    Fits an Autoregressive model of order 3 (AR(3)) to recent prices using Yule-Walker
    equations on centered prices. Predicts the next price using the fitted AR(3) model.
...
def position(df, close, bpy, days=14.0, rebal=7.0):
    """Fit an AR(3) by Yule-Walker on centred prices and take its one-step forecast."""
    w = win(bpy, days)
    ac = autocorr(close, w, 3)
    toe = np.stack([np.stack([ac[:, abs(i - j)] for j in range(3)], axis=1)
                    for i in range(3)], axis=1)
    rhs = ac[:, 1:4]
    ok = np.isfinite(toe).all(axis=(1, 2)) & np.isfinite(rhs).all(axis=1)
    ok &= np.abs(np.linalg.det(np.where(ok[:, None, None], toe, np.eye(3)))) > 1e-12
    phi = np.full((len(close), 3), np.nan)
    if ok.any():
        phi[ok] = np.linalg.solve(toe[ok], rhs[ok][..., None])[..., 0]
    mu = rmean(close, w)
    pred = mu + sum(phi[:, k] * (shift(close, k) - mu) for k in range(3))
