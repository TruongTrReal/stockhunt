"""Revert half the detrended residual when it is large; otherwise extend the trend."""

from __future__ import annotations

import numpy as np

from strategies._forecast import (expose, lin, rmean, win)

RULE = 'Revert half the detrended residual when it is large; otherwise extend the trend.'
SOURCE = "megacellar '164 Trading Strategy Browser' (megacellar-browser.onrender.com), 'CycleAdjustedTrend'"
FAMILY = 'trend'
DRAFT = False
ANCHOR = False
CLASSES = None
...
What the source published
    Detrends prices using a linear fit. If the current residual (cyclical component) is
    large relative to average absolute residual, it predicts a partial reversion (50%)
    of this residual. Otherwise, it extrapolates the linear trend (current + slope).
...
def position(df, close, bpy, days=21.0, rebal=7.0):
    ...
    resid = close - fitted
    avg = rmean(np.abs(resid), w)
    pred = np.where(np.abs(resid) > avg, close - 0.5 * resid, close + slope)
