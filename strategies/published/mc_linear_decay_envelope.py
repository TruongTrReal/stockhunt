"""Snap the price back to a percentage envelope around the SMA when it escapes."""
...
RULE = 'Snap the price back to a percentage envelope around the SMA when it escapes.'
SOURCE = "megacellar '164 Trading Strategy Browser' (megacellar-browser.onrender.com), 'LinearDecayEnvelope'"
FAMILY = 'reversion'
DRAFT = False
ANCHOR = False
CLASSES = None
...
What the source published
    Defines price envelopes around an SMA where the envelope width (`delta`) decays
    linearly based on window recency (`delta * (n-1)/n`). If the price is outside these
    envelopes, it predicts the respective envelope boundary; otherwise, current price.
...
GRID = (
    {'days': 14.0, 'delta': 0.05, 'rebal': 7.0},
    {'days': 7.0, 'delta': 0.05, 'rebal': 7.0},
    {'days': 21.0, 'delta': 0.05, 'rebal': 7.0},
    {'days': 28.0, 'delta': 0.05, 'rebal': 7.0},
    {'days': 30.0, 'delta': 0.05, 'rebal': 30.0},
    {'days': 60.0, 'delta': 0.05, 'rebal': 30.0},
    {'days': 90.0, 'delta': 0.05, 'rebal': 30.0},
    {'days': 180.0, 'delta': 0.05, 'rebal': 30.0},
)


def position(df, close, bpy, days=14.0, delta=0.05, rebal=7.0):
    """Snap the price back to a percentage envelope around the SMA when it escapes."""
    w = win(bpy, days)
    sma = rmean(close, w)
    d = delta * float(w - 1) / float(w)
    up, dn = sma * (1.0 + d), sma * (1.0 - d)
    pred = np.where(close > up, up, np.where(close < dn, dn, close))
