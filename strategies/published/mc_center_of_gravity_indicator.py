"""Take the price at the window's centre-of-gravity bar as the forecast."""
...
RULE = "Take the price at the window's centre-of-gravity bar as the forecast."
SOURCE = "megacellar '164 Trading Strategy Browser' (megacellar-browser.onrender.com), 'CenterOfGravityIndicator'"
FAMILY = 'reversion'
DRAFT = False
ANCHOR = False
CLASSES = None
...
What the source published
    Calculates the Center of Gravity (COG) indicator for the price series (prices
    weighted by their time index from 0 to N-1). The predicted price is the price at the
    COG's (rounded) time index.
...
def position(df, close, bpy, days=28.0, rebal=7.0):
    ...
    cog = safe_div(np.nansum(m * t, axis=1), np.nansum(m, axis=1), fill=np.nan)
    j = np.clip(np.nan_to_num(np.round(cog), nan=0.0).astype("int64"), 0, w - 1)
    pred = np.where(bad, np.nan, m[np.arange(len(m)), j])
