"""Predict the largest absolute price in the window."""

from __future__ import annotations

import numpy as np

from strategies._forecast import (expose, rmax, win)

RULE = 'Predict the largest absolute price in the window.'
SOURCE = "megacellar '164 Trading Strategy Browser' (megacellar-browser.onrender.com), 'RollingWindowLInfinityNorm'"
FAMILY = 'trend'
DRAFT = False
ANCHOR = False
CLASSES = None
NOTE = "RE-IMPLEMENTED FROM A DESCRIPTION, not from source code - the site publishes a one-paragraph account of each rule and no code, so this is a plausible rewrite and not a verified conversion of the original. It is also scored against a different benchmark: the source compares a book that rebalances weekly or monthly against one buy-and-hold number that never rebalances at all, which is the unmatched-schedule failure the root CLAUDE.md lists first. Its published result is not comparable to anything on this sheet. Long on every bar that is not itself the window's high, so it is buy-and-hold minus the new-high days. It ranked 4th on the source leaderboard, which says more about that leaderboard than about the rule."

LOGIC = """
What the source published
    Predicts the maximum absolute price (L-infinity norm or Chebyshev norm) observed in
    the recent window.
