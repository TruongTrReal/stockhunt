"""Lean up when the price sits nearer the window low than the window high."""
...
RULE = 'Lean up when the price sits nearer the window low than the window high.'
SOURCE = "megacellar '164 Trading Strategy Browser' (megacellar-browser.onrender.com), 'DistanceToMinMax'"
FAMILY = 'reversion'
DRAFT = False
ANCHOR = False
CLASSES = None
...
What the source published
    Predicts price movement based on the normalized difference `(dist_to_max -
    dist_to_min) / total_range`, scaled by `distance_scale`. Closer to min implies
    upward prediction.
...
GRID = (
    {'days': 21.0, 'distance_scale': 0.1, 'rebal': 7.0},
    {'days': 7.0, 'distance_scale': 0.1, 'rebal': 7.0},
    {'days': 14.0, 'distance_scale': 0.1, 'rebal': 7.0},
    {'days': 28.0, 'distance_scale': 0.1, 'rebal': 7.0},
    {'days': 30.0, 'distance_scale': 0.1, 'rebal': 30.0},
    {'days': 60.0, 'distance_scale': 0.1, 'rebal': 30.0},
    {'days': 90.0, 'distance_scale': 0.1, 'rebal': 30.0},
    {'days': 180.0, 'distance_scale': 0.1, 'rebal': 30.0},
)


def position(df, close, bpy, days=21.0, distance_scale=0.1, rebal=7.0):
    """Lean up when the price sits nearer the window low than the window high."""
    w = win(bpy, days)
    mn, mx = rmin(close, w), rmax(close, w)
    pred = close * (1.0 + distance_scale
                    * safe_div((mx - close) - (close - mn), mx - mn, fill=np.nan))
