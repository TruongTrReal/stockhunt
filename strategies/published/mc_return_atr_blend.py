"""Fade the window's total return once it is large relative to ATR."""

from __future__ import annotations

import numpy as np

from strategies._forecast import (atr_c, expose, safe_div, shift, win)

RULE = "Fade the window's total return once it is large relative to ATR."
SOURCE = "megacellar '164 Trading Strategy Browser' (megacellar-browser.onrender.com), 'ReturnATRBlend'"
FAMILY = 'reversion'
DRAFT = False
ANCHOR = False
CLASSES = None
...
What the source published
    Calculates the total return over the window and an ATR (from absolute price
    changes). Predicts `current_price * (1 - (total_return / ATR) * 0.1)`, implying
    reversion if the ATR-normalized total return is large.
...
def position(df, close, bpy, days=14.0, rebal=7.0):
    """Fade the window's total return once it is large relative to ATR."""
    w = win(bpy, days)
    tot = safe_div(close, shift(close, w - 1), fill=np.nan) - 1.0
    pred = close * (1.0 - safe_div(tot, atr_c(close, w), fill=np.nan) * 0.1)
