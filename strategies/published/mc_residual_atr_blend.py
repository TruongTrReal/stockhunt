"""Fade the price's residual from its fitted trend, normalised by ATR."""

from __future__ import annotations

import numpy as np

from strategies._forecast import (atr_c, expose, lin, safe_div, win)

RULE = "Fade the price's residual from its fitted trend, normalised by ATR."
SOURCE = "megacellar '164 Trading Strategy Browser' (megacellar-browser.onrender.com), 'ResidualATRBlend'"
FAMILY = 'reversion'
DRAFT = False
ANCHOR = False
CLASSES = None
...
What the source published
    Fits a linear trend to prices. Predicts a price by adjusting the current price
    against its residual from the trend (`current - trend_value_at_current_time`),
    normalized by ATR and scaled by -0.5, encouraging reversion to the trend.
...
def position(df, close, bpy, days=7.0, rebal=7.0):
    ...
    pred = close - 0.5 * safe_div(close - fitted, atr_c(close, w), fill=np.nan)
