"""Fade the price's clipped position inside percentage bands around the SMA."""

from __future__ import annotations

import numpy as np

from strategies._forecast import (expose, rmean, safe_div, win)

RULE = "Fade the price's clipped position inside percentage bands around the SMA."
SOURCE = "megacellar '164 Trading Strategy Browser' (megacellar-browser.onrender.com), 'MomentumWithinBands'"
FAMILY = 'reversion'
DRAFT = False
ANCHOR = False
CLASSES = None
...
What the source published
    Similar to RollingWindowBandMomentum, but the calculated momentum (normalized
    position within SMA-percentage bands) is clipped to ±1 before being used for the
...
GRID = (
    {'days': 7.0, 'pct': 0.05, 'scale': 0.02, 'rebal': 7.0},
    {'days': 14.0, 'pct': 0.05, 'scale': 0.02, 'rebal': 7.0},
    {'days': 21.0, 'pct': 0.05, 'scale': 0.02, 'rebal': 7.0},
    {'days': 28.0, 'pct': 0.05, 'scale': 0.02, 'rebal': 7.0},
    {'days': 30.0, 'pct': 0.05, 'scale': 0.02, 'rebal': 30.0},
    {'days': 60.0, 'pct': 0.05, 'scale': 0.02, 'rebal': 30.0},
    {'days': 90.0, 'pct': 0.05, 'scale': 0.02, 'rebal': 30.0},
    {'days': 180.0, 'pct': 0.05, 'scale': 0.02, 'rebal': 30.0},
)


def position(df, close, bpy, days=7.0, pct=0.05, scale=0.02, rebal=7.0):
    """Fade the price's clipped position inside percentage bands around the SMA."""
    w = win(bpy, days)
    sma = rmean(close, w)
    mom = np.clip(safe_div(close - sma, pct * sma, fill=np.nan), -1.0, 1.0)
    pred = close * (1.0 - mom * scale)
