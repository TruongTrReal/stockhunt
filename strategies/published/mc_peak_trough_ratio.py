"""Extend the last move when peaks outnumber troughs, reverse it otherwise."""
...
RULE = 'Extend the last move when peaks outnumber troughs, reverse it otherwise.'
SOURCE = "megacellar '164 Trading Strategy Browser' (megacellar-browser.onrender.com), 'PeakTroughRatio'"
FAMILY = 'trend'
DRAFT = False
ANCHOR = False
CLASSES = None
...
What the source published
    Calculates the ratio of peaks to troughs. If peaks > troughs, it predicts
    continuation of the last price change (trend). Otherwise (ratio <= 1 or troughs=0),
    it predicts reversal (reversion).
...
def position(df, close, bpy, days=7.0, rebal=7.0):
    ...
    npk, ntr = shift(rsum(pk, w), 1), shift(rsum(tr, w), 1)
    chg = close - shift(close, 1)
    pred = np.where(npk > ntr, close + chg, close - chg)
