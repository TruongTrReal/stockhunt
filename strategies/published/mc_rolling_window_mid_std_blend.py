"""Start at the window mean and lean by the z-score times the median-mean gap."""

from __future__ import annotations

import numpy as np

from strategies._forecast import (expose, rmean, rmedian, rstd, safe_div, win)

RULE = 'Start at the window mean and lean by the z-score times the median-mean gap.'
SOURCE = "megacellar '164 Trading Strategy Browser' (megacellar-browser.onrender.com), 'RollingWindowMidStdBlend'"
FAMILY = 'reversion'
DRAFT = False
ANCHOR = False
CLASSES = None
...
What the source published
    Predicts a price by starting at the window mean and adding an adjustment based on
    the z-score of the current price multiplied by the deviation of the window median
    from the window mean (`mean + z_score * (median - mean)`).
...
def position(df, close, bpy, days=21.0, rebal=7.0):
    """Start at the window mean and lean by the z-score times the median-mean gap."""
    w = win(bpy, days)
    mu, sd, med = rmean(close, w), rstd(close, w), rmedian(close, w)
