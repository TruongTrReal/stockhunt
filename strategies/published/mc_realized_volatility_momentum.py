"""Rising realised volatility means momentum; falling means reversion."""

from __future__ import annotations

import numpy as np

from strategies._forecast import (expose, lin, rets, rmean, rstd, shift, win)

RULE = 'Rising realised volatility means momentum; falling means reversion.'
SOURCE = "megacellar '164 Trading Strategy Browser' (megacellar-browser.onrender.com), 'RealizedVolatilityMomentum'"
FAMILY = 'regime'
DRAFT = False
ANCHOR = False
CLASSES = None
...
What the source published
    Calculates realized volatility over two recent, consecutive sub-windows. If
    volatility is rising (slope > 0), it predicts momentum continuation. If volatility
    is falling or flat, it predicts mean reversion to the window average.
...
def position(df, close, bpy, days=14.0, rebal=7.0):
    ...
    half = max(2, w // 2)
    r = rets(close)
    v_now, v_prev = rstd(r, half), shift(rstd(r, half), half)
    slope, _ = lin(close, w)
    pred = np.where(v_now > v_prev, close + slope, rmean(close, w))
