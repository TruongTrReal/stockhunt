"""Scale the price by one minus its z-score - reversion with no damping at all."""
...
RULE = 'Scale the price by one minus its z-score - reversion with no damping at all.'
SOURCE = "megacellar '164 Trading Strategy Browser' (megacellar-browser.onrender.com), 'NormalizedBandDeviationReversion'"
FAMILY = 'reversion'
DRAFT = False
ANCHOR = False
CLASSES = None
NOTE = "RE-IMPLEMENTED FROM A DESCRIPTION, not from source code - the site publishes a one-paragraph account of each rule and no code, so this is a plausible rewrite and not a verified conversion of the original. It is also scored against a different benchmark: the source compares a book that rebalances weekly or monthly against one buy-and-hold number that never rebalances at all, which is the unmatched-schedule failure the root CLAUDE.md lists first. Its published result is not comparable to anything on this sheet. The z-score is used as a RETURN, so a 2-sigma deviation forecasts a 200% move. Only its sign reaches the position, so the rule is identical to plain 'close below the window mean' - which is what `mc_rolling_window_z_score_mean` also reduces to. Both are kept because the source shipped both."

LOGIC = """
What the source published
    Calculates the z-score (`z`) of the current price relative to the window mean and
    standard deviation. Predicts a price by scaling the current price by `(1 - z)`,
    leading to strong reversion when the price is far from the mean.
...
def position(df, close, bpy, days=28.0, rebal=7.0):
    ...
    pred = close * (1.0 - safe_div(close - mu, sd, fill=np.nan))
