"""Revert when the price is more than one mean-absolute-deviation from the mean."""
...
RULE = 'Revert when the price is more than one mean-absolute-deviation from the mean.'
SOURCE = "megacellar '164 Trading Strategy Browser' (megacellar-browser.onrender.com), 'MeanAbsDeviationScore'"
FAMILY = 'reversion'
DRAFT = False
ANCHOR = False
CLASSES = None
...
What the source published
    Calculates the mean absolute deviation (MAD) from the mean price. If the current
    price's deviation from the mean (normalized by MAD) is large (>1), it predicts
    reversion; otherwise (<=1), it predicts slight trend continuation.
...
def position(df, close, bpy, days=28.0, rebal=7.0):
    ...
    w = win(bpy, days)
    mu = rmean(close, w)
    mad = rmean(np.abs(close - mu), w)
    dev = safe_div(close - mu, mad, fill=np.nan)
    pred = np.where(np.abs(dev) > 1.0, mu, close * (1.0 + 0.01 * np.sign(dev)))
