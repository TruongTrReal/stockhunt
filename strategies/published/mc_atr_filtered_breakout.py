"""Follow a break of the prior range by more than k*ATR; otherwise revert to the mean."""

from __future__ import annotations

import numpy as np

from strategies._forecast import (atr_c, expose, rmax, rmean, rmin, shift, win)

RULE = 'Follow a break of the prior range by more than k*ATR; otherwise revert to the mean.'
SOURCE = "megacellar '164 Trading Strategy Browser' (megacellar-browser.onrender.com), 'ATRFilteredBreakout'"
FAMILY = 'trend'
DRAFT = False
ANCHOR = False
CLASSES = None
...
What the source published
    Approximates ATR from close changes. If the current price breaks out of the recent
    high/low by more than `k * ATR`, it predicts a small (2%) continuation. Otherwise,
    it predicts reversion to the window mean.
...
GRID = (
    {'days': 7.0, 'k': 1.0, 'rebal': 7.0},
    {'days': 14.0, 'k': 1.0, 'rebal': 7.0},
    {'days': 21.0, 'k': 1.0, 'rebal': 7.0},
    {'days': 28.0, 'k': 1.0, 'rebal': 7.0},
    {'days': 30.0, 'k': 1.0, 'rebal': 30.0},
    {'days': 60.0, 'k': 1.0, 'rebal': 30.0},
    {'days': 90.0, 'k': 1.0, 'rebal': 30.0},
    {'days': 180.0, 'k': 1.0, 'rebal': 30.0},
)


def position(df, close, bpy, days=7.0, k=1.0, rebal=7.0):
    """Follow a break of the prior range by more than k*ATR; otherwise revert to the mean."""
    w = win(bpy, days)
    atr = atr_c(close, w)
    hh, ll = shift(rmax(close, w), 1), shift(rmin(close, w), 1)
    mu = rmean(close, w)
    pred = np.where(close > hh + k * atr, close * 1.02,
                    np.where(close < ll - k * atr, close * 0.98, mu))
