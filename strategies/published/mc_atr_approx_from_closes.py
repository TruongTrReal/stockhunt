"""Blend the price toward the window mean, weighted by close-to-close ATR over price."""

from __future__ import annotations

import numpy as np

from strategies._forecast import (atr_c, expose, rmean, safe_div, win)

RULE = 'Blend the price toward the window mean, weighted by close-to-close ATR over price.'
SOURCE = "megacellar '164 Trading Strategy Browser' (megacellar-browser.onrender.com), 'ATRApproxFromCloses'"
FAMILY = 'reversion'
DRAFT = False
ANCHOR = False
CLASSES = None
...
What the source published
    Approximates Average True Range (ATR) using the mean of absolute close-to-close
    changes. Predicts a blend of the current price and the window mean, with weight on
    the mean proportional to `(ATR_approx / current_price)`.
...
def position(df, close, bpy, days=7.0, rebal=7.0):
    """Blend the price toward the window mean, weighted by close-to-close ATR over price."""
    w = win(bpy, days)
    wgt = np.clip(safe_div(atr_c(close, w), close, fill=np.nan), 0.0, 1.0)
    pred = close * (1.0 - wgt) + rmean(close, w) * wgt
