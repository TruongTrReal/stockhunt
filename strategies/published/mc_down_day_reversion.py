"""Scale the price up in proportion to how many of the window's closes were down."""

from __future__ import annotations

import numpy as np

from strategies._forecast import (expose, rsum, safe_div, win)

RULE = "Scale the price up in proportion to how many of the window's closes were down."
SOURCE = "megacellar '164 Trading Strategy Browser' (megacellar-browser.onrender.com), 'DownDayReversion'"
FAMILY = 'reversion'
DRAFT = False
ANCHOR = False
CLASSES = None
NOTE = 'RE-IMPLEMENTED FROM A DESCRIPTION, not from source code - the site publishes a one-paragraph account of each rule and no code, so this is a plausible rewrite and not a verified conversion of the original. It is also scored against a different benchmark: the source compares a book that rebalances weekly or monthly against one buy-and-hold number that never rebalances at all, which is the unmatched-schedule failure the root CLAUDE.md lists first. Its published result is not comparable to anything on this sheet. NEARLY ALWAYS LONG: flat only on the rare window containing no down close at all, so it is a buy-and-hold with a handful of holidays.'

LOGIC = """
What the source published
    Predicts the current price scaled by `(1 + count_of_down_days / window_size)`,
    implying a reversion or bounce after many down days.
...
def position(df, close, bpy, days=14.0, rebal=7.0):
    """Scale the price up in proportion to how many of the window's closes were down."""
    w = win(bpy, days)
    dn = (np.diff(close, prepend=np.nan) < 0).astype("float64")
    pred = close * (1.0 + safe_div(rsum(dn, w), float(w), fill=np.nan))
