"""An exponentially weighted average of recent prices."""
...
RULE = 'An exponentially weighted average of recent prices.'
SOURCE = "megacellar '164 Trading Strategy Browser' (megacellar-browser.onrender.com), 'LiquidityWeightedPrice'"
FAMILY = 'reversion'
DRAFT = False
ANCHOR = False
CLASSES = None
NOTE = "RE-IMPLEMENTED FROM A DESCRIPTION, not from source code - the site publishes a one-paragraph account of each rule and no code, so this is a plausible rewrite and not a verified conversion of the original. It is also scored against a different benchmark: the source compares a book that rebalances weekly or monthly against one buy-and-hold number that never rebalances at all, which is the unmatched-schedule failure the root CLAUDE.md lists first. Its published result is not comparable to anything on this sheet. Despite the name this uses no volume at all in the source - the weighting is recency only. Reproduced as published; the name is the source's."

LOGIC = """
What the source published
    Predicts the next price as an exponentially weighted moving average of recent
    prices, giving more weight to more recent prices (using a decay factor).
