"""Predict a bounce only when the price has fallen through the lower channel band."""
...
RULE = 'Predict a bounce only when the price has fallen through the lower channel band.'
SOURCE = "megacellar '164 Trading Strategy Browser' (megacellar-browser.onrender.com), 'SimpleChannelReversion'"
FAMILY = 'reversion'
DRAFT = False
ANCHOR = False
CLASSES = None
NOTE = 'RE-IMPLEMENTED FROM A DESCRIPTION, not from source code - the site publishes a one-paragraph account of each rule and no code, so this is a plausible rewrite and not a verified conversion of the original. It is also scored against a different benchmark: the source compares a book that rebalances weekly or monthly against one buy-and-hold number that never rebalances at all, which is the unmatched-schedule failure the root CLAUDE.md lists first. Its published result is not comparable to anything on this sheet. The source describes the lower band only and says nothing about the upper one, so the rule sits at the current price everywhere else. It is therefore a one-sided, low duty-cycle dip buyer.'

LOGIC = """
What the source published
    If the current price drops below a lower band (SMA − k·σ), it predicts a reversion
    upwards, with the predicted price scaled by `(1 + (lower_band − current_price) /
    SMA)`.
...
def position(df, close, bpy, days=14.0, k=2.0, rebal=7.0):
    """Predict a bounce only when the price has fallen through the lower channel band."""
    w = win(bpy, days)
    sma, sd = rmean(close, w), rstd(close, w)
    lower = sma - k * sd
    pred = np.where(close < lower,
                    close * (1.0 + safe_div(lower - close, sma, fill=0.0)), close)
