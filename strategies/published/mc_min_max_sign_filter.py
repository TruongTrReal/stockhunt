"""Nudge up near the window low and down near the window high."""
...
RULE = 'Nudge up near the window low and down near the window high.'
SOURCE = "megacellar '164 Trading Strategy Browser' (megacellar-browser.onrender.com), 'MinMaxSignFilter'"
FAMILY = 'reversion'
DRAFT = False
ANCHOR = False
CLASSES = None
...
What the source published
    If the current price is very close (within `near_threshold` of normalized range) to
    the window's minimum, it predicts a small price increase (`prediction_strength`). If
    very close to the maximum, it predicts a small decrease. Otherwise, current price.
...
GRID = (
    {'days': 21.0, 'near_threshold': 0.1, 'strength': 0.02, 'rebal': 7.0},
    {'days': 7.0, 'near_threshold': 0.1, 'strength': 0.02, 'rebal': 7.0},
    {'days': 14.0, 'near_threshold': 0.1, 'strength': 0.02, 'rebal': 7.0},
    {'days': 28.0, 'near_threshold': 0.1, 'strength': 0.02, 'rebal': 7.0},
    {'days': 30.0, 'near_threshold': 0.1, 'strength': 0.02, 'rebal': 30.0},
    {'days': 60.0, 'near_threshold': 0.1, 'strength': 0.02, 'rebal': 30.0},
    {'days': 90.0, 'near_threshold': 0.1, 'strength': 0.02, 'rebal': 30.0},
    {'days': 180.0, 'near_threshold': 0.1, 'strength': 0.02, 'rebal': 30.0},
)


def position(df, close, bpy, days=21.0, near_threshold=0.1, strength=0.02, rebal=7.0):
    """Nudge up near the window low and down near the window high."""
    w = win(bpy, days)
    mn, mx = rmin(close, w), rmax(close, w)
    pos = safe_div(close - mn, mx - mn, fill=np.nan)
    pred = np.where(pos < near_threshold, close * (1.0 + strength),
                    np.where(pos > 1.0 - near_threshold, close * (1.0 - strength), close))
