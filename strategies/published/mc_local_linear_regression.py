"""Fit a line to a short recent sub-window and extend it one bar."""
...
RULE = 'Fit a line to a short recent sub-window and extend it one bar.'
SOURCE = "megacellar '164 Trading Strategy Browser' (megacellar-browser.onrender.com), 'LocalLinearRegression'"
FAMILY = 'trend'
DRAFT = False
ANCHOR = False
CLASSES = None
NOTE = 'RE-IMPLEMENTED FROM A DESCRIPTION, not from source code - the site publishes a one-paragraph account of each rule and no code, so this is a plausible rewrite and not a verified conversion of the original. It is also scored against a different benchmark: the source compares a book that rebalances weekly or monthly against one buy-and-hold number that never rebalances at all, which is the unmatched-schedule failure the root CLAUDE.md lists first. Its published result is not comparable to anything on this sheet. Identical machinery to `mc_linear_trend_extrapolation` at a shorter lookback. The source ships both and they are counted as two trials, because they were.'

LOGIC = """
What the source published
    Fits a linear regression to a small, most recent sub-window (`local_window`) of
    prices and extrapolates this local trend one step forward to predict the next price.
...
GRID = (
    {'local_days': 180.0, 'rebal': 30.0},
    {'local_days': 7.0, 'rebal': 7.0},
    {'local_days': 14.0, 'rebal': 7.0},
    {'local_days': 21.0, 'rebal': 7.0},
    {'local_days': 28.0, 'rebal': 7.0},
    {'local_days': 30.0, 'rebal': 30.0},
    {'local_days': 60.0, 'rebal': 30.0},
    {'local_days': 90.0, 'rebal': 30.0},
)


def position(df, close, bpy, local_days=180.0, rebal=30.0):
    """Fit a line to a short recent sub-window and extend it one bar."""
    w = win(bpy, local_days)
    slope, fitted = lin(close, w)
    pred = fitted + slope
