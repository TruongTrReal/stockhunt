"""Take the double exponential moving average as the forecast."""
...
RULE = 'Take the double exponential moving average as the forecast.'
SOURCE = "megacellar '164 Trading Strategy Browser' (megacellar-browser.onrender.com), 'DEMAProjection'"
FAMILY = 'trend'
DRAFT = False
ANCHOR = False
CLASSES = None
...
What the source published
    Calculates the Double Exponential Moving Average (DEMA) of prices for a given
    period. The predicted price is the latest DEMA value.
...
def position(df, close, bpy, days=7.0, rebal=7.0):
    """Take the double exponential moving average as the forecast."""
    w = win(bpy, days)
    pred = talib.DEMA(np.ascontiguousarray(close), timeperiod=max(2, w))
