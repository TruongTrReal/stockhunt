"""Move the price with the Chaikin oscillator normalised by the A/D line's range."""

from __future__ import annotations

import numpy as np

from strategies._forecast import (ema, expose, rmax, rmin, safe_div, win)

RULE = "Move the price with the Chaikin oscillator normalised by the A/D line's range."
SOURCE = "megacellar '164 Trading Strategy Browser' (megacellar-browser.onrender.com), 'ChaikinOscillatorSpread'"
FAMILY = 'volatility'
DRAFT = False
ANCHOR = False
CLASSES = ('us_stocks', 'us_etfs', 'cme_futures')
NOTE = 'RE-IMPLEMENTED FROM A DESCRIPTION, not from source code - the site publishes a one-paragraph account of each rule and no code, so this is a plausible rewrite and not a verified conversion of the original. It is also scored against a different benchmark: the source compares a book that rebalances weekly or monthly against one buy-and-hold number that never rebalances at all, which is the unmatched-schedule failure the root CLAUDE.md lists first. Its published result is not comparable to anything on this sheet. Volume-gated, same exclusion as `mc_chaikin_money_flow_gradient`.'

LOGIC = """
What the source published
    Calculates the Chaikin Oscillator (difference between 3-period and 10-period EMAs of
    the Accumulation/Distribution Line). The predicted price is adjusted proportionally
    (0.01 factor) to the oscillator value normalized by ADL range.
...
def position(df, close, bpy, days=14.0, rebal=7.0):
    ...
    adl = np.cumsum(safe_div((close - lo) - (hi - close), hi - lo, fill=0.0) * vol)
    osc = ema(adl, span=3) - ema(adl, span=10)
    rng = rmax(adl, w) - rmin(adl, w)
    pred = close * (1.0 + 0.01 * safe_div(osc, rng, fill=np.nan))
