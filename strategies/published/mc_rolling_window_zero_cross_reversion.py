"""Revert to the mean when the cumulative difference path crosses zero often."""
...
RULE = 'Revert to the mean when the cumulative difference path crosses zero often.'
SOURCE = "megacellar '164 Trading Strategy Browser' (megacellar-browser.onrender.com), 'RollingWindowZeroCrossReversion'"
FAMILY = 'regime'
DRAFT = False
ANCHOR = False
CLASSES = None
...
What the source published
    If the cumulative sum of price differences crosses zero frequently (more than
    `cross_threshold` times), it predicts reversion to the window mean. Otherwise, it
    predicts the current price.
...
GRID = (
    {'days': 28.0, 'cross_threshold': 0.2, 'rebal': 7.0},
    {'days': 7.0, 'cross_threshold': 0.2, 'rebal': 7.0},
    {'days': 14.0, 'cross_threshold': 0.2, 'rebal': 7.0},
    {'days': 21.0, 'cross_threshold': 0.2, 'rebal': 7.0},
    {'days': 30.0, 'cross_threshold': 0.2, 'rebal': 30.0},
    {'days': 60.0, 'cross_threshold': 0.2, 'rebal': 30.0},
    {'days': 90.0, 'cross_threshold': 0.2, 'rebal': 30.0},
    {'days': 180.0, 'cross_threshold': 0.2, 'rebal': 30.0},
)


def position(df, close, bpy, days=28.0, cross_threshold=0.2, rebal=7.0):
    """Revert to the mean when the cumulative difference path crosses zero often."""
    w = win(bpy, days)
    d = np.diff(close, prepend=np.nan)
    m = roll(d, w)
    bad = nan_mask(m)
    cs = np.nancumsum(np.where(np.isfinite(m), m, 0.0), axis=1)
    s = np.sign(cs)
    crosses = (s[:, 1:] != s[:, :-1]).sum(axis=1).astype("float64")
    pred = np.where(bad, np.nan,
                    np.where(crosses > cross_threshold * float(w), rmean(close, w), close))
