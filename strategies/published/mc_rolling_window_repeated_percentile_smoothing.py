"""Clip the price series at a high percentile, then take a short average of the result."""

from __future__ import annotations

import numpy as np

from strategies._forecast import (expose, rmean, rquantile, win)

RULE = 'Clip the price series at a high percentile, then take a short average of the result.'
SOURCE = "megacellar '164 Trading Strategy Browser' (megacellar-browser.onrender.com), 'RollingWindowRepeatedPercentileSmoothing'"
FAMILY = 'reversion'
DRAFT = False
ANCHOR = False
CLASSES = None
...
What the source published
    Applies a two-step smoothing: first, prices are clipped if they are above a
    specified percentile (`perc`). Then, a short-term (5-period or less) Simple Moving
    Average of these clipped prices is taken as the prediction.
...
GRID = (
    {'days': 21.0, 'perc': 0.9, 'rebal': 7.0},
    {'days': 7.0, 'perc': 0.9, 'rebal': 7.0},
    {'days': 14.0, 'perc': 0.9, 'rebal': 7.0},
    {'days': 28.0, 'perc': 0.9, 'rebal': 7.0},
    {'days': 30.0, 'perc': 0.9, 'rebal': 30.0},
    {'days': 60.0, 'perc': 0.9, 'rebal': 30.0},
    {'days': 90.0, 'perc': 0.9, 'rebal': 30.0},
    {'days': 180.0, 'perc': 0.9, 'rebal': 30.0},
)


def position(df, close, bpy, days=21.0, perc=0.9, rebal=7.0):
    """Clip the price series at a high percentile, then take a short average of the result."""
    w = win(bpy, days)
    cap = rquantile(close, w, perc)
    pred = rmean(np.minimum(close, cap), max(2, min(5, w)))
