"""Blend a smoothed rolling maximum and minimum by a fixed weight."""

from __future__ import annotations

import numpy as np

from strategies._forecast import (expose, rmax, rmean, rmin, win)

RULE = 'Blend a smoothed rolling maximum and minimum by a fixed weight.'
SOURCE = "megacellar '164 Trading Strategy Browser' (megacellar-browser.onrender.com), 'MinMaxBlend'"
FAMILY = 'reversion'
DRAFT = False
ANCHOR = False
CLASSES = None
...
What the source published
    Calculates smoothed series of rolling maximums and rolling minimums over `period`
    and `smooth_period`. The predicted price is a weighted (`alpha`) blend of these
    smoothed maximum and minimum values.
...
GRID = (
    {'days': 28.0, 'smooth_days': 10.0, 'alpha': 0.5, 'rebal': 7.0},
    {'days': 7.0, 'smooth_days': 10.0, 'alpha': 0.5, 'rebal': 7.0},
    {'days': 14.0, 'smooth_days': 10.0, 'alpha': 0.5, 'rebal': 7.0},
    {'days': 21.0, 'smooth_days': 10.0, 'alpha': 0.5, 'rebal': 7.0},
    {'days': 30.0, 'smooth_days': 10.0, 'alpha': 0.5, 'rebal': 30.0},
    {'days': 60.0, 'smooth_days': 10.0, 'alpha': 0.5, 'rebal': 30.0},
    {'days': 90.0, 'smooth_days': 10.0, 'alpha': 0.5, 'rebal': 30.0},
    {'days': 180.0, 'smooth_days': 10.0, 'alpha': 0.5, 'rebal': 30.0},
)


def position(df, close, bpy, days=28.0, smooth_days=10.0, alpha=0.5, rebal=7.0):
    """Blend a smoothed rolling maximum and minimum by a fixed weight."""
    w = win(bpy, days)
    sm = max(2, win(bpy, smooth_days))
    hi_s, lo_s = rmean(rmax(close, w), sm), rmean(rmin(close, w), sm)
    pred = alpha * hi_s + (1.0 - alpha) * lo_s
