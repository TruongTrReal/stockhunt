"""Fit a line to log(log(price)) to catch super-exponential trends and extend it a bar."""
...
RULE = 'Fit a line to log(log(price)) to catch super-exponential trends and extend it a bar.'
SOURCE = "megacellar '164 Trading Strategy Browser' (megacellar-browser.onrender.com), 'DoubleLogFit'"
FAMILY = 'trend'
DRAFT = False
ANCHOR = False
CLASSES = None
NOTE = "RE-IMPLEMENTED FROM A DESCRIPTION, not from source code - the site publishes a one-paragraph account of each rule and no code, so this is a plausible rewrite and not a verified conversion of the original. It is also scored against a different benchmark: the source compares a book that rebalances weekly or monthly against one buy-and-hold number that never rebalances at all, which is the unmatched-schedule failure the root CLAUDE.md lists first. Its published result is not comparable to anything on this sheet. Undefined where the price is below e, because log(log(p)) is. Those bars are NaN and therefore flat; on this repo's universes that is a handful of penny prints rather than a systematic gap."

LOGIC = """
What the source published
    Fits a linear regression to the double logarithm (log(log(price))) of recent prices
    to model super-exponential trends, then extrapolates and converts back to price,
    with prediction clipped to [0.5*current, 2.0*current].
...
def position(df, close, bpy, days=21.0, rebal=7.0):
    ...
    y = np.log(np.where(lp > 0, lp, np.nan))
    slope, fitted = lin(y, w)
    pred = np.clip(np.exp(np.exp(fitted + slope)), 0.5 * close, 2.0 * close)
