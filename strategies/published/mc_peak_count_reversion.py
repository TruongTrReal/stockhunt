"""Reverse the last move in a choppy window with many peaks; extend it in a quiet one."""
...
RULE = 'Reverse the last move in a choppy window with many peaks; extend it in a quiet one.'
SOURCE = "megacellar '164 Trading Strategy Browser' (megacellar-browser.onrender.com), 'PeakCountReversion'"
FAMILY = 'reversion'
DRAFT = False
ANCHOR = False
CLASSES = None
NOTE = 'RE-IMPLEMENTED FROM A DESCRIPTION, not from source code - the site publishes a one-paragraph account of each rule and no code, so this is a plausible rewrite and not a verified conversion of the original. It is also scored against a different benchmark: the source compares a book that rebalances weekly or monthly against one buy-and-hold number that never rebalances at all, which is the unmatched-schedule failure the root CLAUDE.md lists first. Its published result is not comparable to anything on this sheet. A peak needs the bar AFTER it, so the count is shifted by one bar before use. Without that shift the rule reads a bar it could not have seen, which is the leak `test_causality.py` exists to catch.'

LOGIC = """
What the source published
    Counts local price peaks in the window. If many peaks (>= window_size/4), it
    predicts a reversal of the last price change. If few peaks, it predicts a
