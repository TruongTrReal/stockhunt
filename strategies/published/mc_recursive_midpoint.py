"""Smooth the bar's high-low midpoint recursively and take that as the forecast."""
...
RULE = "Smooth the bar's high-low midpoint recursively and take that as the forecast."
SOURCE = "megacellar '164 Trading Strategy Browser' (megacellar-browser.onrender.com), 'RecursiveMidpoint'"
FAMILY = 'reversion'
DRAFT = False
ANCHOR = False
CLASSES = None
NOTE = 'RE-IMPLEMENTED FROM A DESCRIPTION, not from source code - the site publishes a one-paragraph account of each rule and no code, so this is a plausible rewrite and not a verified conversion of the original. It is also scored against a different benchmark: the source compares a book that rebalances weekly or monthly against one buy-and-hold number that never rebalances at all, which is the unmatched-schedule failure the root CLAUDE.md lists first. Its published result is not comparable to anything on this sheet. A DUPLICATE of `mc_adaptive_midpoint` in the source, differing only in the default alpha. Both are carried because both were separate entries on the source leaderboard and therefore separate trials.'

LOGIC = """
What the source published
    Calculates the midpoint of the current high-low range and then smooths this midpoint
    value recursively using an exponential moving average (with `alpha` as smoothing
    factor). The smoothed midpoint is the prediction.
