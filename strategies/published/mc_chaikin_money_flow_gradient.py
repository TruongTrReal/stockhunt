"""Move the price with the slope of Chaikin Money Flow over the window."""

from __future__ import annotations

import numpy as np

from strategies._forecast import (expose, lin, rsum, safe_div, win)

RULE = 'Move the price with the slope of Chaikin Money Flow over the window.'
SOURCE = "megacellar '164 Trading Strategy Browser' (megacellar-browser.onrender.com), 'ChaikinMoneyFlowGradient'"
FAMILY = 'volatility'
DRAFT = False
ANCHOR = False
CLASSES = ('us_stocks', 'us_etfs', 'cme_futures')
NOTE = 'RE-IMPLEMENTED FROM A DESCRIPTION, not from source code - the site publishes a one-paragraph account of each rule and no code, so this is a plausible rewrite and not a verified conversion of the original. It is also scored against a different benchmark: the source compares a book that rebalances weekly or monthly against one buy-and-hold number that never rebalances at all, which is the unmatched-schedule failure the root CLAUDE.md lists first. Its published result is not comparable to anything on this sheet. Volume-gated. The vendor serves no volume for `crypto` or `commodities` — the column is present and identically zero — so the rule is undefined there and the class is excluded rather than scored as a rule that never trades.'

LOGIC = """
What the source published
    Calculates Chaikin Money Flow (CMF) values over a window. The predicted price is
    adjusted proportionally to the gradient (slope) of the CMF, normalized by total
...
def position(df, close, bpy, days=7.0, rebal=7.0):
    ...
    mfm = safe_div((close - lo) - (hi - close), hi - lo, fill=0.0)
    cmf = safe_div(rsum(mfm * vol, w), rsum(vol, w), fill=np.nan)
    slope, _ = lin(cmf, w)
    pred = close * (1.0 + np.nan_to_num(slope, nan=np.nan))
