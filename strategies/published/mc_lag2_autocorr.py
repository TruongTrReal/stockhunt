"""Blend the price two bars back with the current one, weighted by lag-2 autocorrelation."""
...
RULE = 'Blend the price two bars back with the current one, weighted by lag-2 autocorrelation.'
SOURCE = "megacellar '164 Trading Strategy Browser' (megacellar-browser.onrender.com), 'Lag2Autocorr'"
FAMILY = 'reversion'
DRAFT = False
ANCHOR = False
CLASSES = None
...
What the source published
    Calculates the lag-2 autocorrelation (rho2) of prices. The prediction is a blend:
    `price_t-2 * rho2 + current_price * (1 - abs(rho2))`.
...
def position(df, close, bpy, days=21.0, rebal=7.0):
    """Blend the price two bars back with the current one, weighted by lag-2 autocorrelation."""
    w = win(bpy, days)
    rho2 = autocorr(close, w, 2)[:, 2]
    pred = shift(close, 2) * rho2 + close * (1.0 - np.abs(rho2))
