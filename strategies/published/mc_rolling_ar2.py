"""Fit an AR(2) by Yule-Walker on centred prices and take its one-step forecast."""

from __future__ import annotations

import numpy as np

from strategies._forecast import (autocorr, expose, rmean, safe_div, shift, win)

RULE = 'Fit an AR(2) by Yule-Walker on centred prices and take its one-step forecast.'
SOURCE = "megacellar '164 Trading Strategy Browser' (megacellar-browser.onrender.com), 'RollingAR2'"
FAMILY = 'reversion'
DRAFT = False
ANCHOR = False
CLASSES = None
...
What the source published
    Fits an Autoregressive model of order 2 (AR(2)) to recent prices using Yule-Walker
    equations on centered prices. Predicts the next price using the fitted AR(2) model.
...
def position(df, close, bpy, days=14.0, rebal=7.0):
    """Fit an AR(2) by Yule-Walker on centred prices and take its one-step forecast."""
    w = win(bpy, days)
    ac = autocorr(close, w, 2)
    r1, r2 = ac[:, 1], ac[:, 2]
    den = 1.0 - r1 * r1
    p1 = safe_div(r1 * (1.0 - r2), den, fill=np.nan)
    p2 = safe_div(r2 - r1 * r1, den, fill=np.nan)
    mu = rmean(close, w)
    pred = mu + p1 * (close - mu) + p2 * (shift(close, 1) - mu)
