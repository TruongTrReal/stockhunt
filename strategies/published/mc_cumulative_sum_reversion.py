"""Revert toward the mean, offset by the window's CUSUM of deviations from it."""

from __future__ import annotations

import numpy as np

from strategies._forecast import (expose, nan_mask, rmean, roll, win)

RULE = "Revert toward the mean, offset by the window's CUSUM of deviations from it."
SOURCE = "megacellar '164 Trading Strategy Browser' (megacellar-browser.onrender.com), 'CumulativeSumReversion'"
FAMILY = 'reversion'
DRAFT = False
ANCHOR = False
CLASSES = None
NOTE = "RE-IMPLEMENTED FROM A DESCRIPTION, not from source code - the site publishes a one-paragraph account of each rule and no code, so this is a plausible rewrite and not a verified conversion of the original. It is also scored against a different benchmark: the source compares a book that rebalances weekly or monthly against one buy-and-hold number that never rebalances at all, which is the unmatched-schedule failure the root CLAUDE.md lists first. Its published result is not comparable to anything on this sheet. AMBIGUOUS SOURCE, resolved and recorded. Taken literally, the CUSUM of deviations from the window's own mean is exactly zero at the window's last bar, which would collapse the rule to plain mean reversion. The path's MEAN is used instead, which is the nearest reading that keeps the CUSUM doing any work. This is a re-implementation choice, not the source's."

LOGIC = """
What the source published
    Calculates the cumulative sum (CUSUM) of price deviations from the window mean.
    Predicts a price that reverts towards the mean, adjusted by the normalized CUSUM
    value (`mean - cusum / N`).
...
def position(df, close, bpy, days=21.0, rebal=7.0):
    ...
    m = roll(close, w)
    bad = nan_mask(m)
    dev = m - np.where(bad[:, None], np.nan, mu[:, None])
    cs = np.nancumsum(np.where(np.isfinite(dev), dev, 0.0), axis=1)
    pred = np.where(bad, np.nan, mu - cs.mean(axis=1) / float(w))
