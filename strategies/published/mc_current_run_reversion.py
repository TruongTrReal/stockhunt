"""Fade the current run of up-closes, in proportion to its length."""
...
RULE = 'Fade the current run of up-closes, in proportion to its length.'
SOURCE = "megacellar '164 Trading Strategy Browser' (megacellar-browser.onrender.com), 'CurrentRunReversion'"
FAMILY = 'reversion'
DRAFT = False
ANCHOR = False
CLASSES = None
NOTE = 'RE-IMPLEMENTED FROM A DESCRIPTION, not from source code - the site publishes a one-paragraph account of each rule and no code, so this is a plausible rewrite and not a verified conversion of the original. It is also scored against a different benchmark: the source compares a book that rebalances weekly or monthly against one buy-and-hold number that never rebalances at all, which is the unmatched-schedule failure the root CLAUDE.md lists first. Its published result is not comparable to anything on this sheet. STRUCTURALLY ALWAYS FLAT under the long/flat mapping. The run length is non-negative, so the forecast is never above the close and the rule never takes a position. Kept and reported rather than dropped: it was a trial, and an always-flat cell is information about the MAPPING, not about the rule.'

LOGIC = """
What the source published
    Identifies the length of the current consecutive run of 'up days'. Predicts the
    current price scaled by `(1 - run_length / window_size)`, implying reversion after
    long positive runs.
...
def position(df, close, bpy, days=30.0, rebal=30.0):
    """Fade the current run of up-closes, in proportion to its length."""
    w = win(bpy, days)
    up = (np.diff(close, prepend=np.nan) > 0).astype("float64")
    run = np.zeros(len(close))
    for i in range(1, len(close)):
        run[i] = run[i - 1] + 1.0 if up[i] else 0.0
    pred = close * (1.0 - run / float(w))
