"""Extend above the 75th percentile, revert to the median below the 25th."""

from __future__ import annotations

import numpy as np

from strategies._forecast import (expose, rmedian, rquantile, win)

RULE = 'Extend above the 75th percentile, revert to the median below the 25th.'
SOURCE = "megacellar '164 Trading Strategy Browser' (megacellar-browser.onrender.com), 'MidPercentileCrossover'"
FAMILY = 'trend'
DRAFT = False
ANCHOR = False
CLASSES = None
...
What the source published
    If the current price is above the 75th percentile of the window, it predicts trend
    continuation (scaled by deviation from 75th). If below the 25th percentile, it
    predicts reversion to the 50th percentile (median). Otherwise, current price.
...
def position(df, close, bpy, days=21.0, rebal=7.0):
    """Extend above the 75th percentile, revert to the median below the 25th."""
    w = win(bpy, days)
    q25, q50, q75 = (rquantile(close, w, 0.25), rmedian(close, w),
                     rquantile(close, w, 0.75))
    pred = np.where(close > q75, close + (close - q75),
                    np.where(close < q25, q50, close))
