"""Above an ADX threshold, follow +DI minus -DI; below it, revert to the window mean."""
...
from strategies._forecast import (expose, rmean, win)

RULE = 'Above an ADX threshold, follow +DI minus -DI; below it, revert to the window mean.'
SOURCE = "megacellar '164 Trading Strategy Browser' (megacellar-browser.onrender.com), 'ADXThresholdTrend'"
FAMILY = 'trend'
DRAFT = False
ANCHOR = False
CLASSES = None
...
What the source published
    Calculates the Average Directional Index (ADX). If ADX exceeds a specified
    threshold, it predicts trend continuation based on the difference between Positive
    Directional Indicator (+DI) and Negative Directional Indicator (-DI). Otherwise, it
    predicts the mean of recent prices.
...
GRID = (
    {'days': 90.0, 'smooth': 14.0, 'threshold': 25.0, 'rebal': 30.0},
    {'days': 7.0, 'smooth': 14.0, 'threshold': 25.0, 'rebal': 7.0},
    {'days': 14.0, 'smooth': 14.0, 'threshold': 25.0, 'rebal': 7.0},
    {'days': 21.0, 'smooth': 14.0, 'threshold': 25.0, 'rebal': 7.0},
    {'days': 28.0, 'smooth': 14.0, 'threshold': 25.0, 'rebal': 7.0},
    {'days': 30.0, 'smooth': 14.0, 'threshold': 25.0, 'rebal': 30.0},
    {'days': 60.0, 'smooth': 14.0, 'threshold': 25.0, 'rebal': 30.0},
    {'days': 180.0, 'smooth': 14.0, 'threshold': 25.0, 'rebal': 30.0},
)


def position(df, close, bpy, days=90.0, smooth=14.0, threshold=25.0, rebal=30.0):
    """Above an ADX threshold, follow +DI minus -DI; below it, revert to the window mean."""
    w = win(bpy, days)
    n = win(bpy, smooth)
    hi, lo = df["High"].to_numpy("float64"), df["Low"].to_numpy("float64")
    adx = talib.ADX(hi, lo, close, timeperiod=n)
    pdi = talib.PLUS_DI(hi, lo, close, timeperiod=n)
    mdi = talib.MINUS_DI(hi, lo, close, timeperiod=n)
    trend = close * (1.0 + (pdi - mdi) / 100.0)
    pred = np.where(adx > threshold, trend, rmean(close, w))
