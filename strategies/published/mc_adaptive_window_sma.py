"""A simple moving average whose length grows with the volatility of returns."""

from __future__ import annotations

import numpy as np

from strategies._forecast import (expose, pick, rets, rmean, rstd, win)

RULE = 'A simple moving average whose length grows with the volatility of returns.'
SOURCE = "megacellar '164 Trading Strategy Browser' (megacellar-browser.onrender.com), 'AdaptiveWindowSMA'"
FAMILY = 'reversion'
DRAFT = False
ANCHOR = False
CLASSES = None
NOTE = 'RE-IMPLEMENTED FROM A DESCRIPTION, not from source code - the site publishes a one-paragraph account of each rule and no code, so this is a plausible rewrite and not a verified conversion of the original. It is also scored against a different benchmark: the source compares a book that rebalances weekly or monthly against one buy-and-hold number that never rebalances at all, which is the unmatched-schedule failure the root CLAUDE.md lists first. Its published result is not comparable to anything on this sheet. Same quantisation deviation as `mc_adaptive_std_band`: four candidate lengths instead of a continuously varying one.'

LOGIC = """
What the source published
    Calculates a Simple Moving Average (SMA) where the window length is adaptive,
    determined by `max(2, min(int(std_dev_returns * k), len_prices))`. Higher return
    volatility leads to a larger window, up to `k` times the standard deviation.
...
GRID = (
    {'days': 7.0, 'k': 1000.0, 'rebal': 7.0},
    {'days': 14.0, 'k': 1000.0, 'rebal': 7.0},
    {'days': 21.0, 'k': 1000.0, 'rebal': 7.0},
    {'days': 28.0, 'k': 1000.0, 'rebal': 7.0},
    {'days': 30.0, 'k': 1000.0, 'rebal': 30.0},
    {'days': 60.0, 'k': 1000.0, 'rebal': 30.0},
    {'days': 90.0, 'k': 1000.0, 'rebal': 30.0},
    {'days': 180.0, 'k': 1000.0, 'rebal': 30.0},
)


def position(df, close, bpy, days=7.0, k=1000.0, rebal=7.0):
    """A simple moving average whose length grows with the volatility of returns."""
    w = win(bpy, days)
    vol = rstd(rets(close), w)
    want = np.clip(np.nan_to_num(vol * k, nan=0.0), 2.0, float(w))
    ws = [max(2, int(w * f)) for f in (0.1, 0.25, 0.5, 1.0)]
    mas = [rmean(close, n) for n in ws]
    idx = np.clip(np.digitize(want, [ws[1], ws[2], ws[3]]), 0, 3)
    pred = np.where(np.isfinite(vol), pick(mas, idx), np.nan)
