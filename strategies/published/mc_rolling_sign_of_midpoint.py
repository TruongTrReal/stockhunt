"""Step against the side of the range midpoint the price is on."""
...
RULE = 'Step against the side of the range midpoint the price is on.'
SOURCE = "megacellar '164 Trading Strategy Browser' (megacellar-browser.onrender.com), 'RollingSignOfMidpoint'"
FAMILY = 'reversion'
DRAFT = False
ANCHOR = False
CLASSES = None
...
What the source published
    Predicts mean reversion based on whether the current price is above or below the
    midpoint of the window's high-low range. Predicted price = `current * (1 - delta *
    sign(current - mid))`.
...
GRID = (
    {'days': 60.0, 'delta': 0.01, 'rebal': 30.0},
    {'days': 7.0, 'delta': 0.01, 'rebal': 7.0},
    {'days': 14.0, 'delta': 0.01, 'rebal': 7.0},
    {'days': 21.0, 'delta': 0.01, 'rebal': 7.0},
    {'days': 28.0, 'delta': 0.01, 'rebal': 7.0},
    {'days': 30.0, 'delta': 0.01, 'rebal': 30.0},
    {'days': 90.0, 'delta': 0.01, 'rebal': 30.0},
    {'days': 180.0, 'delta': 0.01, 'rebal': 30.0},
)


def position(df, close, bpy, days=60.0, delta=0.01, rebal=30.0):
    """Step against the side of the range midpoint the price is on."""
    w = win(bpy, days)
    mid = (rmax(close, w) + rmin(close, w)) / 2.0
    pred = close * (1.0 - delta * np.sign(close - mid))
