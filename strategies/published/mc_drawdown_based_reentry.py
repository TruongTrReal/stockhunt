"""Buy back half of any drawdown from the window peak that exceeds a threshold."""

from __future__ import annotations

import numpy as np

from strategies._forecast import (expose, rmax, safe_div, win)

RULE = 'Buy back half of any drawdown from the window peak that exceeds a threshold.'
SOURCE = "megacellar '164 Trading Strategy Browser' (megacellar-browser.onrender.com), 'DrawdownBasedReentry'"
FAMILY = 'reversion'
DRAFT = False
ANCHOR = False
CLASSES = None
...
What the source published
    Identifies if an asset is in a drawdown exceeding a specified threshold from its
    recent peak. If so, it predicts a price bounce equal to half the magnitude of the
    current drawdown.
...
def position(df, close, bpy, days=180.0, threshold=0.1, rebal=30.0):
    """Buy back half of any drawdown from the window peak that exceeds a threshold."""
    w = win(bpy, days)
    peak = rmax(close, w)
    dd = safe_div(peak - close, peak, fill=np.nan)
    pred = np.where(dd > threshold, close * (1.0 + dd / 2.0), close)
