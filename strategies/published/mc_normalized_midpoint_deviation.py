"""Fade the price's deviation from the window's high-low midpoint, scaled by sigma."""

from __future__ import annotations

import numpy as np

from strategies._forecast import (expose, rmax, rmin, rstd, safe_div, win)

RULE = "Fade the price's deviation from the window's high-low midpoint, scaled by sigma."
SOURCE = "megacellar '164 Trading Strategy Browser' (megacellar-browser.onrender.com), 'NormalizedMidpointDeviation'"
FAMILY = 'reversion'
DRAFT = False
ANCHOR = False
CLASSES = None
...
What the source published
    Predicts mean reversion based on the current price's deviation from the window's
    high-low midpoint, normalized by the window's standard deviation. Predicted price =
    `current * (1 - 0.1 * (current - mid) / sigma)`.
...
def position(df, close, bpy, days=21.0, rebal=7.0):
    ...
    sd = rstd(close, w)
    pred = close * (1.0 - 0.1 * safe_div(close - mid, sd, fill=np.nan))
