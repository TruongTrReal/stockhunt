"""An exponentially weighted p-norm of the window's prices."""
...
RULE = "An exponentially weighted p-norm of the window's prices."
SOURCE = "megacellar '164 Trading Strategy Browser' (megacellar-browser.onrender.com), 'RollingWindowWeightedLpNorm'"
FAMILY = 'trend'
DRAFT = False
ANCHOR = False
CLASSES = None
...
What the source published
    Predicts a value based on an exponentially weighted p-th root of the average of the
    p-th powers of absolute prices in the window.
...
GRID = (
    {'days': 7.0, 'p': 2.0, 'decay_factor': 0.94, 'rebal': 7.0},
    {'days': 14.0, 'p': 2.0, 'decay_factor': 0.94, 'rebal': 7.0},
    {'days': 21.0, 'p': 2.0, 'decay_factor': 0.94, 'rebal': 7.0},
    {'days': 28.0, 'p': 2.0, 'decay_factor': 0.94, 'rebal': 7.0},
    {'days': 30.0, 'p': 2.0, 'decay_factor': 0.94, 'rebal': 30.0},
    {'days': 60.0, 'p': 2.0, 'decay_factor': 0.94, 'rebal': 30.0},
    {'days': 90.0, 'p': 2.0, 'decay_factor': 0.94, 'rebal': 30.0},
    {'days': 180.0, 'p': 2.0, 'decay_factor': 0.94, 'rebal': 30.0},
)


def position(df, close, bpy, days=7.0, p=2.0, decay_factor=0.94, rebal=7.0):
    """An exponentially weighted p-norm of the window's prices."""
    w = win(bpy, days)
    age = np.arange(w - 1, -1, -1, dtype="float64")
    wts = decay_factor ** age
    wts = wts / wts.sum()
    m = np.abs(roll(close, w))
    bad = nan_mask(m)
    pred = np.where(bad, np.nan,
                    np.nansum(np.nan_to_num(m, nan=0.0) ** p * wts, axis=1) ** (1.0 / p))
