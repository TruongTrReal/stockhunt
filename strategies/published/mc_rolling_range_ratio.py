"""Strong reversion outside the 30-70 band of the range, a weak pull inside it."""
...
RULE = 'Strong reversion outside the 30-70 band of the range, a weak pull inside it.'
SOURCE = "megacellar '164 Trading Strategy Browser' (megacellar-browser.onrender.com), 'RollingRangeRatio'"
FAMILY = 'reversion'
DRAFT = False
ANCHOR = False
CLASSES = None
...
What the source published
    Determines the current price's normalized position (`ratio`) within its recent high-
    low range. If at an extreme of the range (ratio >0.7 or <0.3), it predicts strong
    mean reversion (2% move); otherwise, a weaker reversion (10% towards midpoint).
...
def position(df, close, bpy, days=21.0, rebal=7.0):
    ...
    ratio = safe_div(close - mn, mx - mn, fill=np.nan)
    mid = (mn + mx) / 2.0
    pred = np.where(ratio > 0.7, close * 0.98,
                    np.where(ratio < 0.3, close * 1.02, close + 0.1 * (mid - close)))
