"""The smoothed centre of a recursively updated envelope."""
...
RULE = 'The smoothed centre of a recursively updated envelope.'
SOURCE = "megacellar '164 Trading Strategy Browser' (megacellar-browser.onrender.com), 'RecursiveEnvelope'"
FAMILY = 'reversion'
DRAFT = False
ANCHOR = False
CLASSES = None
NOTE = 'NAMED BY THE SOURCE AS A TOP-3 STRATEGY, advertised as RecursiveEnvelope at a 180-day lookback rebalanced monthly, claiming 20.38% CAGR after fees (132.1x total against a 9.75x benchmark). RE-IMPLEMENTED FROM A DESCRIPTION, not from source code - the site publishes a one-paragraph account of each rule and no code, so this is a plausible rewrite and not a verified conversion of the original. It is also scored against a different benchmark: the source compares a book that rebalances weekly or monthly against one buy-and-hold number that never rebalances at all, which is the unmatched-schedule failure the root CLAUDE.md lists first. Its published result is not comparable to anything on this sheet. The source updates a WIDTH alongside the centre and then predicts the centre alone, so the width is computed and discarded. Reproduced as published; the rule is an EMA crossover in everything but name.'

LOGIC = """
What the source published
    Recursively updates an envelope's center (EMA of current price with `alpha`) and its
    width (EMA of `recent_vol / center` with `alpha`). Predicts the current smoothed
    envelope center.
...
def position(df, close, bpy, days=180.0, rebal=30.0):
    """The smoothed centre of a recursively updated envelope."""
    pred = ema(close, span=win(bpy, days))
