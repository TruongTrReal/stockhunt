"""Take the fitted trend's intercept - its level at the window's start - as the forecast."""
...
RULE = "Take the fitted trend's intercept - its level at the window's start - as the forecast."
SOURCE = "megacellar '164 Trading Strategy Browser' (megacellar-browser.onrender.com), 'InterceptOnlyTrend'"
FAMILY = 'reversion'
DRAFT = False
ANCHOR = False
CLASSES = None
NOTE = "RE-IMPLEMENTED FROM A DESCRIPTION, not from source code - the site publishes a one-paragraph account of each rule and no code, so this is a plausible rewrite and not a verified conversion of the original. It is also scored against a different benchmark: the source compares a book that rebalances weekly or monthly against one buy-and-hold number that never rebalances at all, which is the unmatched-schedule failure the root CLAUDE.md lists first. Its published result is not comparable to anything on this sheet. The intercept is the trend line at the OLDEST bar in the window, so this is long whenever the fitted line has fallen across the window. It is a slow downtrend-buying rule wearing a regression's clothes."

LOGIC = """
What the source published
    Fits a linear trend to recent prices. The predicted price is the intercept of this
    fitted line (clamped to be > 0.01), representing a perceived base level from the
    start of the window.
...
def position(df, close, bpy, days=7.0, rebal=7.0):
    ...
    pred = np.maximum(fitted - slope * float(w - 1), 0.01)
