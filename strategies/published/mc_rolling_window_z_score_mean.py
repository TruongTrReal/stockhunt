"""Scale the price by one minus its z-score against the window."""
...
RULE = 'Scale the price by one minus its z-score against the window.'
SOURCE = "megacellar '164 Trading Strategy Browser' (megacellar-browser.onrender.com), 'RollingWindowZScoreMean'"
FAMILY = 'reversion'
DRAFT = False
ANCHOR = False
CLASSES = None
NOTE = "RE-IMPLEMENTED FROM A DESCRIPTION, not from source code - the site publishes a one-paragraph account of each rule and no code, so this is a plausible rewrite and not a verified conversion of the original. It is also scored against a different benchmark: the source compares a book that rebalances weekly or monthly against one buy-and-hold number that never rebalances at all, which is the unmatched-schedule failure the root CLAUDE.md lists first. Its published result is not comparable to anything on this sheet. Reduces under this mapping to 'long below the window mean', identically to `mc_normalized_band_deviation_reversion`. The source ships both as separate entries and both are carried, so the trial count reflects what was searched."

LOGIC = """
What the source published
    Calculates the z-score of the current price relative to the window mean and standard
    deviation. Predicts a price by scaling the current price by `(1 - z-score)`,
    implying strong mean reversion.
