"""Revert toward the mean in proportion to the window's Ulcer Index."""
...
RULE = "Revert toward the mean in proportion to the window's Ulcer Index."
SOURCE = "megacellar '164 Trading Strategy Browser' (megacellar-browser.onrender.com), 'UlcerIndexReversion'"
FAMILY = 'reversion'
DRAFT = False
ANCHOR = False
CLASSES = None
...
What the source published
    Calculates the Ulcer Index (measures depth and duration of drawdowns). Predicts mean
    reversion with strength proportional to the Ulcer Index value, blended with the
    current price.
...
GRID = (
    {'days': 7.0, 'scale': 5.0, 'rebal': 7.0},
    {'days': 14.0, 'scale': 5.0, 'rebal': 7.0},
    {'days': 21.0, 'scale': 5.0, 'rebal': 7.0},
    {'days': 28.0, 'scale': 5.0, 'rebal': 7.0},
    {'days': 30.0, 'scale': 5.0, 'rebal': 30.0},
    {'days': 60.0, 'scale': 5.0, 'rebal': 30.0},
    {'days': 90.0, 'scale': 5.0, 'rebal': 30.0},
    {'days': 180.0, 'scale': 5.0, 'rebal': 30.0},
)


def position(df, close, bpy, days=7.0, scale=5.0, rebal=7.0):
    """Revert toward the mean in proportion to the window's Ulcer Index."""
    w = win(bpy, days)
    m = roll(close, w)
    bad = nan_mask(m)
    ...
    ui = np.where(bad, np.nan, np.sqrt((dd * dd).mean(axis=1)))
    pull = np.clip(ui * scale, 0.0, 1.0)
    pred = close + pull * (rmean(close, w) - close)
