"""Scale the price down by a tenth of its z-score against the window."""
...
RULE = 'Scale the price down by a tenth of its z-score against the window.'
SOURCE = "megacellar '164 Trading Strategy Browser' (megacellar-browser.onrender.com), 'CloseRangeOscillator'"
FAMILY = 'reversion'
DRAFT = False
ANCHOR = False
CLASSES = None
...
What the source published
    Calculates an oscillator based on the current price's deviation from the mean,
    normalized by standard deviation (z-score). Predicts mean reversion by scaling
    current price by `(1 - oscillator * 0.1)`.
...
def position(df, close, bpy, days=28.0, rebal=7.0):
    ...
    pred = close * (1.0 - safe_div(close - mu, sd, fill=np.nan) * 0.1)
