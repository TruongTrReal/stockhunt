"""Drift 30% of the way toward a smoothed 'max pain' level."""
...
RULE = "Drift 30% of the way toward a smoothed 'max pain' level."
SOURCE = "megacellar '164 Trading Strategy Browser' (megacellar-browser.onrender.com), 'MaxPainPivot'"
FAMILY = 'reversion'
DRAFT = False
ANCHOR = False
CLASSES = None
...
What the source published
    Uses an exponentially weighted moving average of recent prices as a proxy for a 'max
    pain' level. Predicts a price that moves part of the way (30%) from the current
    price towards this calculated max pain value.
...
def position(df, close, bpy, days=7.0, rebal=7.0):
    """Drift 30% of the way toward a smoothed 'max pain' level."""
    mp = ema(close, span=win(bpy, days))
    pred = close + 0.3 * (mp - close)
