"""Above the median predict the 75th percentile, below it the 25th."""
...
RULE = 'Above the median predict the 75th percentile, below it the 25th.'
SOURCE = "megacellar '164 Trading Strategy Browser' (megacellar-browser.onrender.com), 'QuantileMomentum'"
FAMILY = 'trend'
DRAFT = False
ANCHOR = False
CLASSES = None
...
What the source published
    If the current price is above the recent median, it predicts the 75th percentile
    price from the window. If below or equal to the median, it predicts the 25th
    percentile.
...
def position(df, close, bpy, days=7.0, rebal=7.0):
    ...
    pred = np.where(close > q50, q75, q25)
