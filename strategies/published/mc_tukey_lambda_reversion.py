"""Revert to the median only when the window's quantiles look symmetric."""

from __future__ import annotations

import numpy as np

from strategies._forecast import (expose, rmedian, rquantile, safe_div, win)

RULE = "Revert to the median only when the window's quantiles look symmetric."
SOURCE = "megacellar '164 Trading Strategy Browser' (megacellar-browser.onrender.com), 'TukeyLambdaReversion'"
FAMILY = 'reversion'
DRAFT = False
ANCHOR = False
CLASSES = None
NOTE = 'RE-IMPLEMENTED FROM A DESCRIPTION, not from source code - the site publishes a one-paragraph account of each rule and no code, so this is a plausible rewrite and not a verified conversion of the original. It is also scored against a different benchmark: the source compares a book that rebalances weekly or monthly against one buy-and-hold number that never rebalances at all, which is the unmatched-schedule failure the root CLAUDE.md lists first. Its published result is not comparable to anything on this sheet. The shape parameter is estimated as the log ratio of the two quartile gaps, which is zero for a symmetric window and signed by its skew. The source does not give its estimator, so this is a re-implementation choice.'

LOGIC = """
What the source published
    Estimates the shape parameter (lambda) of a Tukey lambda distribution from window
    quantiles (Q1, Median, Q3). If the distribution appears symmetric (lambda near zero,
    |lambda|<0.5), it predicts reversion to the median; otherwise, it predicts the
    current price.
...
def position(df, close, bpy, days=7.0, rebal=7.0):
    """Revert to the median only when the window's quantiles look symmetric."""
    w = win(bpy, days)
    q1, med, q3 = (rquantile(close, w, 0.25), rmedian(close, w),
                   rquantile(close, w, 0.75))
    lam = np.log(safe_div(q3 - med, med - q1, fill=np.nan))
    pred = np.where(np.abs(lam) < 0.5, med, close)
