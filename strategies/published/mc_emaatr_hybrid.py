"""Predict the ATR band on whichever side of the EMA the price is already on."""
...
RULE = 'Predict the ATR band on whichever side of the EMA the price is already on.'
SOURCE = "megacellar '164 Trading Strategy Browser' (megacellar-browser.onrender.com), 'EMAATRHybrid'"
FAMILY = 'trend'
DRAFT = False
ANCHOR = False
CLASSES = None
...
What the source published
    Calculates an EMA and an ATR (from absolute price changes). Predicts the upper ATR
    band around the EMA if current price is above EMA, and the lower ATR band if current
    price is below EMA.
...
def position(df, close, bpy, days=7.0, rebal=7.0):
    ...
    pred = np.where(close > e, e + atr, e - atr)
