"""Revert to the mean when the window's average drawdown clears a threshold."""

from __future__ import annotations

import numpy as np

from strategies._forecast import (expose, nan_mask, rmean, roll, safe_div, win)

RULE = "Revert to the mean when the window's average drawdown clears a threshold."
SOURCE = "megacellar '164 Trading Strategy Browser' (megacellar-browser.onrender.com), 'PainIndexReversion'"
FAMILY = 'reversion'
DRAFT = False
ANCHOR = False
CLASSES = None
...
What the source published
    Calculates the Pain Index (average drawdown over the window). If the Pain Index is
    above a threshold (0.1), it predicts reversion to the mean price; otherwise, it
...
GRID = (
    {'days': 60.0, 'threshold': 0.1, 'rebal': 30.0},
    {'days': 7.0, 'threshold': 0.1, 'rebal': 7.0},
    {'days': 14.0, 'threshold': 0.1, 'rebal': 7.0},
    {'days': 21.0, 'threshold': 0.1, 'rebal': 7.0},
    {'days': 28.0, 'threshold': 0.1, 'rebal': 7.0},
    {'days': 30.0, 'threshold': 0.1, 'rebal': 30.0},
    {'days': 90.0, 'threshold': 0.1, 'rebal': 30.0},
    {'days': 180.0, 'threshold': 0.1, 'rebal': 30.0},
)


def position(df, close, bpy, days=60.0, threshold=0.1, rebal=30.0):
    """Revert to the mean when the window's average drawdown clears a threshold."""
    w = win(bpy, days)
    m = roll(close, w)
    bad = nan_mask(m)
    peak = np.maximum.accumulate(np.nan_to_num(m, nan=-np.inf), axis=1)
    dd = safe_div(peak - m, peak, fill=0.0)
    pain = np.where(bad, np.nan, dd.mean(axis=1))
    pred = np.where(pain > threshold, rmean(close, w), close)
