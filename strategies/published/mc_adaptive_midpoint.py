"""Smooth the bar's own high-low midpoint and take that as the forecast."""
...
RULE = "Smooth the bar's own high-low midpoint and take that as the forecast."
SOURCE = "megacellar '164 Trading Strategy Browser' (megacellar-browser.onrender.com), 'AdaptiveMidpoint'"
FAMILY = 'reversion'
DRAFT = False
ANCHOR = False
CLASSES = None
NOTE = 'RE-IMPLEMENTED FROM A DESCRIPTION, not from source code - the site publishes a one-paragraph account of each rule and no code, so this is a plausible rewrite and not a verified conversion of the original. It is also scored against a different benchmark: the source compares a book that rebalances weekly or monthly against one buy-and-hold number that never rebalances at all, which is the unmatched-schedule failure the root CLAUDE.md lists first. Its published result is not comparable to anything on this sheet. No lookback window at all, so like `ibs` a 4h version is a genuinely different statistic rather than a rescaled daily one. Long whenever the close sits below the smoothed midpoint, which is a close-in-lower-half rule with memory.'

LOGIC = """
What the source published
    Calculates the midpoint of the current high-low range and then smooths this midpoint
    value over time for each asset independently using an exponential moving average
    (with `alpha` as smoothing factor). The smoothed midpoint is the prediction.
...
def position(df, close, bpy, days=21.0, rebal=7.0):
    ...
    pred = ema((hi + lo) / 2.0, span=win(bpy, days))
