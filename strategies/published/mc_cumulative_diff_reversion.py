"""Subtract the slope of the window's cumulative price differences from the price."""
...
RULE = "Subtract the slope of the window's cumulative price differences from the price."
SOURCE = "megacellar '164 Trading Strategy Browser' (megacellar-browser.onrender.com), 'CumulativeDiffReversion'"
FAMILY = 'reversion'
DRAFT = False
ANCHOR = False
CLASSES = None
...
What the source published
    Calculates the slope of the cumulative sum of price differences within the window.
    Predicts the current price minus this slope, effectively mean-reverting against the
    trend in cumulative differences.
...
def position(df, close, bpy, days=60.0, rebal=30.0):
    ...
    slope = np.where(bad, np.nan, (cs * (t - tbar)).sum(axis=1) / sxx)
    pred = close - slope
