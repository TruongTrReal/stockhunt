"""Ehlers' two-pole SuperSmoother, taken as the forecast."""

from __future__ import annotations

import numpy as np

from strategies._forecast import (expose, iir2, win)

RULE = "Ehlers' two-pole SuperSmoother, taken as the forecast."
SOURCE = "megacellar '164 Trading Strategy Browser' (megacellar-browser.onrender.com), 'EhlersSuperSmoother'"
FAMILY = 'trend'
DRAFT = False
ANCHOR = False
CLASSES = None
...
What the source published
    Applies Ehlers' SuperSmoother filter, a low-lag filter, to recent prices using
    coefficients derived from a cutoff period. The predicted price is the latest value
    from the smoothed series.
...
def position(df, close, bpy, days=7.0, rebal=7.0):
    """Ehlers' two-pole SuperSmoother, taken as the forecast."""
    n = max(2.0, float(win(bpy, days)))
    a1 = float(np.exp(-1.414 * np.pi / n))
    b1 = 2.0 * a1 * float(np.cos(1.414 * np.pi / n))
    c2, c3 = b1, -a1 * a1
    pred = iir2(close, 1.0 - c2 - c3, c2, c3)
