"""Outside a percentage envelope predict its boundary; inside it predict the SMA."""
...
RULE = 'Outside a percentage envelope predict its boundary; inside it predict the SMA.'
SOURCE = "megacellar '164 Trading Strategy Browser' (megacellar-browser.onrender.com), 'RollingWindowEnvelope'"
FAMILY = 'reversion'
DRAFT = False
ANCHOR = False
CLASSES = None
...
What the source published
    Defines price envelopes as a percentage (`envelope_pct`) above and below a Simple
    Moving Average (SMA). If the price is outside the envelope, it predicts the
    respective envelope boundary; otherwise, it predicts the SMA.
...
GRID = (
    {'days': 21.0, 'envelope_pct': 0.05, 'rebal': 7.0},
    {'days': 7.0, 'envelope_pct': 0.05, 'rebal': 7.0},
    {'days': 14.0, 'envelope_pct': 0.05, 'rebal': 7.0},
    {'days': 28.0, 'envelope_pct': 0.05, 'rebal': 7.0},
    {'days': 30.0, 'envelope_pct': 0.05, 'rebal': 30.0},
    {'days': 60.0, 'envelope_pct': 0.05, 'rebal': 30.0},
    {'days': 90.0, 'envelope_pct': 0.05, 'rebal': 30.0},
    {'days': 180.0, 'envelope_pct': 0.05, 'rebal': 30.0},
)


def position(df, close, bpy, days=21.0, envelope_pct=0.05, rebal=7.0):
    """Outside a percentage envelope predict its boundary; inside it predict the SMA."""
    w = win(bpy, days)
    sma = rmean(close, w)
    up, dn = sma * (1.0 + envelope_pct), sma * (1.0 - envelope_pct)
    pred = np.where(close > up, up, np.where(close < dn, dn, sma))
