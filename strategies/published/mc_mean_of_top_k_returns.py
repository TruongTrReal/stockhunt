"""Scale the price up by the mean of the window's best quarter of positive returns."""

from __future__ import annotations

import numpy as np

from strategies._forecast import (expose, nan_mask, rets, roll, rowquantile, win)

RULE = "Scale the price up by the mean of the window's best quarter of positive returns."
SOURCE = "megacellar '164 Trading Strategy Browser' (megacellar-browser.onrender.com), 'MeanOfTopKReturns'"
FAMILY = 'trend'
DRAFT = False
ANCHOR = False
CLASSES = None
NOTE = 'RE-IMPLEMENTED FROM A DESCRIPTION, not from source code - the site publishes a one-paragraph account of each rule and no code, so this is a plausible rewrite and not a verified conversion of the original. It is also scored against a different benchmark: the source compares a book that rebalances weekly or monthly against one buy-and-hold number that never rebalances at all, which is the unmatched-schedule failure the root CLAUDE.md lists first. Its published result is not comparable to anything on this sheet. NEARLY ALWAYS LONG: the average of positive returns is non-negative, so the forecast falls to the close only on a window with no up bar at all.'

LOGIC = """
What the source published
    Calculates the average of the top 25% of positive returns in the window (or 0 if no
    positive returns). The predicted price is the current price scaled by (1 + this
    average top return).
...
def position(df, close, bpy, days=21.0, rebal=7.0):
    ...
    cut = rowquantile(pos, 0.75)
    top = np.where(pos >= cut[:, None], pos, np.nan)
    cnt = np.isfinite(top).sum(axis=1)
    avg = np.where(cnt > 0, np.nansum(np.where(np.isfinite(top), top, 0.0), axis=1)
                   / np.where(cnt > 0, cnt, 1), 0.0)
    pred = np.where(bad, np.nan, close * (1.0 + avg))
