"""A fixed blend of the window's maximum and minimum price."""

from __future__ import annotations

import numpy as np

from strategies._forecast import (expose, rmax, rmin, win)

RULE = "A fixed blend of the window's maximum and minimum price."
SOURCE = "megacellar '164 Trading Strategy Browser' (megacellar-browser.onrender.com), 'RollingMinMaxWeighted'"
FAMILY = 'reversion'
DRAFT = False
ANCHOR = False
CLASSES = None
NOTE = "RE-IMPLEMENTED FROM A DESCRIPTION, not from source code - the site publishes a one-paragraph account of each rule and no code, so this is a plausible rewrite and not a verified conversion of the original. It is also scored against a different benchmark: the source compares a book that rebalances weekly or monthly against one buy-and-hold number that never rebalances at all, which is the unmatched-schedule failure the root CLAUDE.md lists first. Its published result is not comparable to anything on this sheet. At alpha 0.5 this is the Donchian midpoint, so the rule is 'long in the lower half of the range' - the same statistic `ibs` computes inside a single bar, measured over a window instead."

LOGIC = """
What the source published
    Predicts a price that is a weighted average of the window's maximum (`alpha` weight)
    and minimum prices (`1-alpha` weight).
...
GRID = (
    {'days': 7.0, 'alpha': 0.5, 'rebal': 7.0},
    {'days': 14.0, 'alpha': 0.5, 'rebal': 7.0},
    {'days': 21.0, 'alpha': 0.5, 'rebal': 7.0},
    {'days': 28.0, 'alpha': 0.5, 'rebal': 7.0},
    {'days': 30.0, 'alpha': 0.5, 'rebal': 30.0},
    {'days': 60.0, 'alpha': 0.5, 'rebal': 30.0},
    {'days': 90.0, 'alpha': 0.5, 'rebal': 30.0},
    {'days': 180.0, 'alpha': 0.5, 'rebal': 30.0},
)


def position(df, close, bpy, days=7.0, alpha=0.5, rebal=7.0):
    """A fixed blend of the window's maximum and minimum price."""
    w = win(bpy, days)
    pred = alpha * rmax(close, w) + (1.0 - alpha) * rmin(close, w)
