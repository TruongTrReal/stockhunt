"""Revert to the mean past an ATR band calibrated to hold a target fraction of history."""

from __future__ import annotations

import numpy as np

from strategies._forecast import (atr_c, expose, nan_mask, rmean, roll, rowquantile, safe_div, win)

RULE = 'Revert to the mean past an ATR band calibrated to hold a target fraction of history.'
SOURCE = "megacellar '164 Trading Strategy Browser' (megacellar-browser.onrender.com), 'DynamicATRBand'"
FAMILY = 'reversion'
DRAFT = False
ANCHOR = False
CLASSES = None
...
What the source published
    Dynamically determines a multiplier `k` such that a `target_in_band` percentage of
    historical price deviations from mean (normalized by ATR) fall within `k*ATR`. If
    current deviation exceeds this, predicts mean; otherwise, partial (20%) reversion.
...
GRID = (
    {'days': 21.0, 'target_in_band': 0.8, 'rebal': 7.0},
    {'days': 7.0, 'target_in_band': 0.8, 'rebal': 7.0},
    {'days': 14.0, 'target_in_band': 0.8, 'rebal': 7.0},
    {'days': 28.0, 'target_in_band': 0.8, 'rebal': 7.0},
    {'days': 30.0, 'target_in_band': 0.8, 'rebal': 30.0},
    {'days': 60.0, 'target_in_band': 0.8, 'rebal': 30.0},
    {'days': 90.0, 'target_in_band': 0.8, 'rebal': 30.0},
    {'days': 180.0, 'target_in_band': 0.8, 'rebal': 30.0},
)


def position(df, close, bpy, days=21.0, target_in_band=0.8, rebal=7.0):
    """Revert to the mean past an ATR band calibrated to hold a target fraction of history."""
    w = win(bpy, days)
    mu, atr = rmean(close, w), atr_c(close, w)
    ratio = np.abs(safe_div(close - mu, atr, fill=np.nan))
    m = roll(ratio, w)
    bad = nan_mask(m)
    k = rowquantile(m, target_in_band)
    pred = np.where(ratio > k, mu, close + 0.2 * (mu - close))
