"""Extrapolate the fitted trend only when its slope clears its own standard error."""

from __future__ import annotations

import numpy as np

from strategies._forecast import (expose, lin, roll, rowmean, safe_div, win)

RULE = 'Extrapolate the fitted trend only when its slope clears its own standard error.'
SOURCE = "megacellar '164 Trading Strategy Browser' (megacellar-browser.onrender.com), 'AdaptiveSlopeFilter'"
FAMILY = 'trend'
DRAFT = False
ANCHOR = False
CLASSES = None
...
What the source published
    Fits a linear trend to prices. If the trend's slope is statistically significant
    (exceeds `sigma_mult` times its standard error), it predicts trend continuation.
    Otherwise, it predicts the current price.
...
GRID = (
    {'days': 30.0, 'sigma_mult': 2.0, 'rebal': 30.0},
    {'days': 7.0, 'sigma_mult': 2.0, 'rebal': 7.0},
    {'days': 14.0, 'sigma_mult': 2.0, 'rebal': 7.0},
    {'days': 21.0, 'sigma_mult': 2.0, 'rebal': 7.0},
    {'days': 28.0, 'sigma_mult': 2.0, 'rebal': 7.0},
    {'days': 60.0, 'sigma_mult': 2.0, 'rebal': 30.0},
    {'days': 90.0, 'sigma_mult': 2.0, 'rebal': 30.0},
    {'days': 180.0, 'sigma_mult': 2.0, 'rebal': 30.0},
)


def position(df, close, bpy, days=30.0, sigma_mult=2.0, rebal=30.0):
    """Extrapolate the fitted trend only when its slope clears its own standard error."""
    w = win(bpy, days)
    ...
    ybar = rowmean(m)
    resid = m - (ybar[:, None] + slope[:, None] * (t - tbar))
    s2 = np.nansum(resid ** 2, axis=1) / max(1.0, w - 2.0)
    se = np.sqrt(safe_div(s2, sxx, fill=np.nan))
    pred = np.where(np.abs(slope) > sigma_mult * se, close + slope, close)
