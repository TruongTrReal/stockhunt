"""Clip the price into a mean +/- k*sigma band and predict the clipped value."""
...
RULE = 'Clip the price into a mean +/- k*sigma band and predict the clipped value.'
SOURCE = "megacellar '164 Trading Strategy Browser' (megacellar-browser.onrender.com), 'RollingOutlierClip'"
FAMILY = 'reversion'
DRAFT = False
ANCHOR = False
CLASSES = None
NOTE = 'RE-IMPLEMENTED FROM A DESCRIPTION, not from source code - the site publishes a one-paragraph account of each rule and no code, so this is a plausible rewrite and not a verified conversion of the original. It is also scored against a different benchmark: the source compares a book that rebalances weekly or monthly against one buy-and-hold number that never rebalances at all, which is the unmatched-schedule failure the root CLAUDE.md lists first. Its published result is not comparable to anything on this sheet. Only ever long when the price is BELOW the lower clip, so it trades on roughly the 2.5% of bars a normal tail would give. A very low duty cycle, which the exposure controls in `strat_wf` will penalise on IR regardless of skill.'

LOGIC = """
What the source published
    Predicts a price by clipping the current price to a range defined by `mean ± k *
    standard_deviations` of the recent window.
...
def position(df, close, bpy, days=21.0, k=2.0, rebal=7.0):
    """Clip the price into a mean +/- k*sigma band and predict the clipped value."""
    w = win(bpy, days)
    mu, sd = rmean(close, w), rstd(close, w)
    pred = np.clip(close, mu - k * sd, mu + k * sd)
