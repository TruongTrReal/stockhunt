"""Revert to the mean outside classic Bollinger bands; sit still inside them."""
...
RULE = 'Revert to the mean outside classic Bollinger bands; sit still inside them.'
SOURCE = "megacellar '164 Trading Strategy Browser' (megacellar-browser.onrender.com), 'RollingWindowMeanStdBand'"
FAMILY = 'reversion'
DRAFT = False
ANCHOR = False
CLASSES = None
...
What the source published
    Uses classical Bollinger Bands (`mean ± std_multiplier * std`). If the current price
    is outside the bands, it predicts reversion to the mean. Otherwise, it predicts the
    current price (no change).
...
GRID = (
    {'days': 7.0, 'std_multiplier': 2.0, 'rebal': 7.0},
    {'days': 14.0, 'std_multiplier': 2.0, 'rebal': 7.0},
    {'days': 21.0, 'std_multiplier': 2.0, 'rebal': 7.0},
    {'days': 28.0, 'std_multiplier': 2.0, 'rebal': 7.0},
    {'days': 30.0, 'std_multiplier': 2.0, 'rebal': 30.0},
    {'days': 60.0, 'std_multiplier': 2.0, 'rebal': 30.0},
    {'days': 90.0, 'std_multiplier': 2.0, 'rebal': 30.0},
    {'days': 180.0, 'std_multiplier': 2.0, 'rebal': 30.0},
)


def position(df, close, bpy, days=7.0, std_multiplier=2.0, rebal=7.0):
    """Revert to the mean outside classic Bollinger bands; sit still inside them."""
    w = win(bpy, days)
    mu, sd = rmean(close, w), rstd(close, w)
    outside = np.abs(close - mu) > std_multiplier * sd
    pred = np.where(outside, mu, close)
