"""Partial reversion against a z-score measured over a window that widens with volatility."""
...
from strategies._forecast import (expose, pick, rets, rmean, rstd, safe_div, win)

RULE = 'Partial reversion against a z-score measured over a window that widens with volatility.'
SOURCE = "megacellar '164 Trading Strategy Browser' (megacellar-browser.onrender.com), 'AdaptiveStdBand'"
FAMILY = 'reversion'
DRAFT = False
ANCHOR = False
CLASSES = None
NOTE = "RE-IMPLEMENTED FROM A DESCRIPTION, not from source code - the site publishes a one-paragraph account of each rule and no code, so this is a plausible rewrite and not a verified conversion of the original. It is also scored against a different benchmark: the source compares a book that rebalances weekly or monthly against one buy-and-hold number that never rebalances at all, which is the unmatched-schedule failure the root CLAUDE.md lists first. Its published result is not comparable to anything on this sheet. The source varies the window continuously with volatility. A per-bar window length cannot be vectorised, so it is quantised to four candidates selected by the volatility's own rolling percentile. That is a deviation from the source."

LOGIC = """
What the source published
    Uses an adaptive window size (inversely related to return volatility) to calculate
    mean, std, and z-score (`(current-mean)/std`). Predicts mean reversion: `current -
    z_score * std * 0.3`.
...
def position(df, close, bpy, days=7.0, rebal=7.0):
    ...
    q = pd.Series(vol).rolling(w).rank(pct=True).to_numpy()
    ws = [max(3, int(w * f)) for f in (0.25, 0.5, 1.0, 2.0)]
    mus = [rmean(close, k) for k in ws]
    sds = [rstd(close, k) for k in ws]
    idx = np.clip(np.digitize(np.nan_to_num(q, nan=0.0), [0.25, 0.5, 0.75]), 0, 3)
    mu, sd = pick(mus, idx), pick(sds, idx)
    z = safe_div(close - mu, sd, fill=np.nan)
    pred = np.where(np.isfinite(q), close - z * sd * 0.3, np.nan)
