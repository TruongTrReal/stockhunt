"""Add the window's min-to-max range spread over its length to the price."""
...
RULE = "Add the window's min-to-max range spread over its length to the price."
SOURCE = "megacellar '164 Trading Strategy Browser' (megacellar-browser.onrender.com), 'MinMaxSlope'"
FAMILY = 'trend'
DRAFT = False
ANCHOR = False
CLASSES = None
NOTE = 'RE-IMPLEMENTED FROM A DESCRIPTION, not from source code - the site publishes a one-paragraph account of each rule and no code, so this is a plausible rewrite and not a verified conversion of the original. It is also scored against a different benchmark: the source compares a book that rebalances weekly or monthly against one buy-and-hold number that never rebalances at all, which is the unmatched-schedule failure the root CLAUDE.md lists first. Its published result is not comparable to anything on this sheet. STRUCTURALLY ALWAYS LONG: max minus min is non-negative by definition, so the forecast is never below the close.'

LOGIC = """
What the source published
    Calculates an average slope from the window's minimum to its maximum price (spread
    over the window length). Predicts the current price plus this slope.
...
def position(df, close, bpy, days=7.0, rebal=7.0):
    """Add the window's min-to-max range spread over its length to the price."""
    w = win(bpy, days)
    pred = close + safe_div(rmax(close, w) - rmin(close, w), float(w), fill=np.nan)
