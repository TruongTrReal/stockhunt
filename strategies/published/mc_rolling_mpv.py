"""Predict the modal price of the window - the midpoint of its busiest histogram bin."""

from __future__ import annotations

import numpy as np

from strategies._forecast import (expose, nan_mask, roll, rowmax, rowmin, win)

RULE = 'Predict the modal price of the window - the midpoint of its busiest histogram bin.'
SOURCE = "megacellar '164 Trading Strategy Browser' (megacellar-browser.onrender.com), 'RollingMPV'"
FAMILY = 'reversion'
DRAFT = False
ANCHOR = False
CLASSES = None
...
What the source published
    Estimates the mode (Most Probable Value - MPV) of prices within the recent window
    using a histogram with a fixed number of bins. The midpoint of the modal bin is the
    predicted price.
...
GRID = (
    {'days': 14.0, 'bins': 20.0, 'rebal': 7.0},
    {'days': 7.0, 'bins': 20.0, 'rebal': 7.0},
    {'days': 21.0, 'bins': 20.0, 'rebal': 7.0},
    {'days': 28.0, 'bins': 20.0, 'rebal': 7.0},
    {'days': 30.0, 'bins': 20.0, 'rebal': 30.0},
    {'days': 60.0, 'bins': 20.0, 'rebal': 30.0},
    {'days': 90.0, 'bins': 20.0, 'rebal': 30.0},
    {'days': 180.0, 'bins': 20.0, 'rebal': 30.0},
)


def position(df, close, bpy, days=14.0, bins=20.0, rebal=7.0):
    """Predict the modal price of the window - the midpoint of its busiest histogram bin."""
    w = win(bpy, days)
    nb = max(2, int(bins))
    m = roll(close, w)
    bad = nan_mask(m)
    lo, hi = rowmin(m), rowmax(m)
    span = np.where(hi > lo, hi - lo, np.nan)
    code = np.floor(np.clip((m - lo[:, None]) / span[:, None] * nb, 0, nb - 1e-9))
    counts = np.stack([(code == k).sum(axis=1) for k in range(nb)], axis=1)
    k = np.argmax(counts, axis=1)
    pred = np.where(bad, np.nan, lo + (k + 0.5) * span / nb)
