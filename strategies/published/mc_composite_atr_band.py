"""Revert to a blended EMA/SMA centre, fully outside an ATR band and 30% inside it."""

from __future__ import annotations

import numpy as np

from strategies._forecast import (atr_c, ema, expose, rmean, win)

RULE = 'Revert to a blended EMA/SMA centre, fully outside an ATR band and 30% inside it.'
SOURCE = "megacellar '164 Trading Strategy Browser' (megacellar-browser.onrender.com), 'CompositeATRBand'"
FAMILY = 'reversion'
DRAFT = False
ANCHOR = False
CLASSES = None
...
What the source published
    Defines ATR-based bands around a composite center (blended EMA and SMA). If current
    price is outside (`center ± band_multiplier * ATR`), predicts reversion to this
    center; otherwise, a partial (30%) reversion.
...
def position(df, close, bpy, days=7.0, band_multiplier=2.0, rebal=7.0):
    """Revert to a blended EMA/SMA centre, fully outside an ATR band and 30% inside it."""
    w = win(bpy, days)
    centre = 0.5 * ema(close, span=w) + 0.5 * rmean(close, w)
    atr = atr_c(close, w)
    far = np.abs(close - centre) > band_multiplier * atr
    pred = np.where(far, centre, close + 0.3 * (centre - close))
