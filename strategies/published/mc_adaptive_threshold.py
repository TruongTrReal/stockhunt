"""Revert to the mean past a z-threshold that widens with the kurtosis of returns."""
...
RULE = 'Revert to the mean past a z-threshold that widens with the kurtosis of returns.'
SOURCE = "megacellar '164 Trading Strategy Browser' (megacellar-browser.onrender.com), 'AdaptiveThreshold'"
FAMILY = 'reversion'
DRAFT = False
ANCHOR = False
CLASSES = None
...
What the source published
    Similar to MeanReversionThreshold, but the threshold multiplier `k` is adaptively
    set based on the kurtosis of recent returns (clamped, min 1.0). High kurtosis allows
    for larger deviations before reversion is predicted.
...
def position(df, close, bpy, days=21.0, rebal=7.0):
    ...
    kurt = pd.Series(rets(close)).rolling(w).kurt().to_numpy()
    k = np.clip(kurt, 1.0, 10.0)
    z = safe_div(close - mu, sd, fill=np.nan)
    pred = np.where(np.abs(z) > k, mu, close)
