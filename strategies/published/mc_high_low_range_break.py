"""Follow a break of the k-bar range in proportion to the breach; else take its midpoint."""

from __future__ import annotations

import numpy as np

from strategies._forecast import (expose, rmax, rmin, safe_div, shift, win)

RULE = 'Follow a break of the k-bar range in proportion to the breach; else take its midpoint.'
SOURCE = "megacellar '164 Trading Strategy Browser' (megacellar-browser.onrender.com), 'HighLowRangeBreak'"
FAMILY = 'trend'
DRAFT = False
ANCHOR = False
CLASSES = None
...
What the source published
    Identifies a k-day price range. If the current price breaks out of this range, it
    predicts further movement in the breakout direction, proportional to the breach size
    relative to the range. Otherwise, it predicts reversion to the range midpoint.
...
def position(df, close, bpy, days=14.0, rebal=7.0):
    ...
    rng = hh - ll
    pred = np.where(close > hh, close * (1.0 + safe_div(close - hh, rng, fill=np.nan)),
                    np.where(close < ll,
                             close * (1.0 - safe_div(ll - close, rng, fill=np.nan)),
                             (hh + ll) / 2.0))
