"""Predict the window mean once at least two peaks have formed in it."""

from __future__ import annotations

import numpy as np

from strategies._forecast import (expose, rmean, rsum, shift, win)

RULE = 'Predict the window mean once at least two peaks have formed in it.'
SOURCE = "megacellar '164 Trading Strategy Browser' (megacellar-browser.onrender.com), 'PeakToPeakForecast'"
FAMILY = 'reversion'
DRAFT = False
ANCHOR = False
CLASSES = None
NOTE = 'RE-IMPLEMENTED FROM A DESCRIPTION, not from source code - the site publishes a one-paragraph account of each rule and no code, so this is a plausible rewrite and not a verified conversion of the original. It is also scored against a different benchmark: the source compares a book that rebalances weekly or monthly against one buy-and-hold number that never rebalances at all, which is the unmatched-schedule failure the root CLAUDE.md lists first. Its published result is not comparable to anything on this sheet. The source computes the peak-to-peak DISTANCE and then does not use it. Reproduced as published rather than repaired - a fixed version of a rule is not a replication of it.'

LOGIC = """
What the source published
    Identifies price peaks in the window. If at least two peaks are found, it predicts
    the mean price of the window. (The distance between peaks is calculated but not used
    in the prediction shown).
...
def position(df, close, bpy, days=21.0, rebal=7.0):
    """Predict the window mean once at least two peaks have formed in it."""
    w = win(bpy, days)
    pk = np.zeros(len(close))
    pk[1:-1] = ((close[1:-1] > close[:-2]) & (close[1:-1] > close[2:])).astype("float64")
    seen = shift(rsum(pk, w), 1)
    pred = np.where(seen >= 2.0, rmean(close, w), close)
