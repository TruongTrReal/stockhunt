"""Recursive least squares with a forgetting factor, extended one bar."""

from __future__ import annotations

import numpy as np

from strategies._forecast import (ewls, expose, win)

RULE = 'Recursive least squares with a forgetting factor, extended one bar.'
SOURCE = "megacellar '164 Trading Strategy Browser' (megacellar-browser.onrender.com), 'RecursiveLeastSquares'"
FAMILY = 'trend'
DRAFT = False
ANCHOR = False
CLASSES = None
NOTE = 'RE-IMPLEMENTED FROM A DESCRIPTION, not from source code - the site publishes a one-paragraph account of each rule and no code, so this is a plausible rewrite and not a verified conversion of the original. It is also scored against a different benchmark: the source compares a book that rebalances weekly or monthly against one buy-and-hold number that never rebalances at all, which is the unmatched-schedule failure the root CLAUDE.md lists first. Its published result is not comparable to anything on this sheet. RLS is implemented in closed form rather than as a per-bar update loop: with the bar index as the only regressor its normal equations are four EWMAs, so the recursion collapses exactly. Same answer, no loop. See `_forecast.ewls`.'

LOGIC = """
What the source published
    Uses Recursive Least Squares (RLS) with a forgetting factor to continuously update a
    linear trend model for prices over the window. Predicts the next price by
    extrapolating this RLS-fitted trend.
...
def position(df, close, bpy, days=7.0, rebal=7.0):
    """Recursive least squares with a forgetting factor, extended one bar."""
    n = float(win(bpy, days))
    slope, fitted = ewls(close, 1.0 - 1.0 / n)
    pred = fitted + slope
