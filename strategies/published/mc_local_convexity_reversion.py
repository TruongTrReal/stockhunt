"""Revert to the mean when local curvature turns negative."""
...
RULE = 'Revert to the mean when local curvature turns negative.'
SOURCE = "megacellar '164 Trading Strategy Browser' (megacellar-browser.onrender.com), 'LocalConvexityReversion'"
FAMILY = 'reversion'
DRAFT = False
ANCHOR = False
CLASSES = None
...
What the source published
    Estimates local price curvature (average of last 3 second-order differences). If
    curvature is negative (suggesting a potential top), it predicts reversion to the
    window mean. Otherwise, it predicts the current price.
...
def position(df, close, bpy, days=28.0, rebal=7.0):
    ...
    curv = rmean(d2, 3)
    pred = np.where(curv < 0, rmean(close, w), close)
