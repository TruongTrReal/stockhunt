"""The window's median with linearly increasing weight on the more recent bars."""
...
RULE = "The window's median with linearly increasing weight on the more recent bars."
SOURCE = "megacellar '164 Trading Strategy Browser' (megacellar-browser.onrender.com), 'WeightedMedianScore'"
FAMILY = 'reversion'
DRAFT = False
ANCHOR = False
CLASSES = None
...
What the source published
    Calculates a weighted median of recent prices, giving linearly increasing weight to
    more recent observations. This weighted median is the predicted price.
...
def position(df, close, bpy, days=28.0, rebal=7.0):
    """The window's median with linearly increasing weight on the more recent bars."""
    w = win(bpy, days)
    wts = np.arange(1.0, w + 1.0)
    wts = wts / wts.sum()
    m = roll(close, w)
    bad = nan_mask(m)
    order = np.argsort(np.nan_to_num(m, nan=np.inf), axis=1)
    rows = np.arange(len(m))[:, None]
    sorted_w = np.cumsum(wts[order], axis=1)
    j = (sorted_w >= 0.5).argmax(axis=1)
    pred = np.where(bad, np.nan, m[rows[:, 0], order[rows[:, 0], j]])
