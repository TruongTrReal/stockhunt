"""Nudge the window median by where the price sits inside the interquartile range."""
...
RULE = 'Nudge the window median by where the price sits inside the interquartile range.'
SOURCE = "megacellar '164 Trading Strategy Browser' (megacellar-browser.onrender.com), 'InterquartileRangeReversion'"
FAMILY = 'reversion'
DRAFT = False
ANCHOR = False
CLASSES = None
...
What the source published
    Predicts a price based on the median of the window, adjusted by a factor `(1 + 0.1 *
    (0.5 - (current-Q1)/IQR))`, encouraging reversion towards the center of the
...
def position(df, close, bpy, days=28.0, rebal=7.0):
    ...
    pos = safe_div(close - q1, q3 - q1, fill=np.nan)
    pred = med * (1.0 + 0.1 * (0.5 - pos))
