"""Revert at percentile extremes, otherwise scale by the percentile itself."""
...
RULE = 'Revert at percentile extremes, otherwise scale by the percentile itself.'
SOURCE = "megacellar '164 Trading Strategy Browser' (megacellar-browser.onrender.com), 'PricePercentileScore'"
FAMILY = 'reversion'
DRAFT = False
ANCHOR = False
CLASSES = None
...
What the source published
    Determines the percentile rank of the current price within its recent window. If the
    price is at an extreme percentile (>0.8 or <0.2), it predicts reversion. Otherwise,
    it scales the current price by a score derived from its percentile (score =
    (percentile-0.5)*2 + 1).
...
def position(df, close, bpy, days=7.0, rebal=7.0):
    """Revert at percentile extremes, otherwise scale by the percentile itself."""
    w = win(bpy, days)
    pct = pd.Series(close).rolling(w).rank(pct=True).to_numpy()
    extreme = (pct > 0.8) | (pct < 0.2)
    pred = np.where(extreme, rmean(close, w), close * ((pct - 0.5) * 2.0 + 1.0))
