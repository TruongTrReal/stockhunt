"""Revert to the mean when second differences become unusually variable."""

from __future__ import annotations

import numpy as np

from strategies._forecast import (expose, rmean, rmedian, rstd, shift, win)

RULE = 'Revert to the mean when second differences become unusually variable.'
SOURCE = "megacellar '164 Trading Strategy Browser' (megacellar-browser.onrender.com), 'RollingWindowIrregularityIndex'"
FAMILY = 'regime'
DRAFT = False
ANCHOR = False
CLASSES = None
NOTE = 'RE-IMPLEMENTED FROM A DESCRIPTION, not from source code - the site publishes a one-paragraph account of each rule and no code, so this is a plausible rewrite and not a verified conversion of the original. It is also scored against a different benchmark: the source compares a book that rebalances weekly or monthly against one buy-and-hold number that never rebalances at all, which is the unmatched-schedule failure the root CLAUDE.md lists first. Its published result is not comparable to anything on this sheet. The threshold is the rolling median of the variance itself. A fixed one is not comparable across assets, and a whole-series median would be look-ahead - the `nanmedian` class of leak this repo has already been bitten by once.'

LOGIC = """
What the source published
    If the variance of second-order price differences (a measure of irregularity) is
    high (above `var_threshold` or median variance), it predicts reversion to the window
    mean. Otherwise, it predicts the current price.
...
def position(df, close, bpy, days=21.0, rebal=7.0):
    ...
    v = rstd(d2, w) ** 2
    cut = rmedian(v, w)
    pred = np.where(v > cut, rmean(close, w), close)
