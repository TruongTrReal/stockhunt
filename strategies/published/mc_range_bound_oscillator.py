"""Strong reversion at the range extremes, a weak pull to the midpoint in between."""
...
RULE = 'Strong reversion at the range extremes, a weak pull to the midpoint in between.'
SOURCE = "megacellar '164 Trading Strategy Browser' (megacellar-browser.onrender.com), 'RangeBoundOscillator'"
FAMILY = 'reversion'
DRAFT = False
ANCHOR = False
CLASSES = None
...
What the source published
    Normalizes the current price within its recent high-low range ([0,1]). If the price
    is at an extreme of this normalized range (>0.8 or <0.2), it predicts strong mean
    reversion (2% move). Otherwise, a slight reversion (10%) towards the range midpoint.
...
def position(df, close, bpy, days=21.0, rebal=7.0):
    ...
    mid = (mn + mx) / 2.0
    pred = np.where(pos > 0.8, close * 0.98,
                    np.where(pos < 0.2, close * 1.02, close + 0.1 * (mid - close)))
