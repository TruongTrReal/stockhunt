"""Fade only returns beyond k sigma; sit at the current price otherwise."""

from __future__ import annotations

import numpy as np

from strategies._forecast import (expose, rets, rstd, win)

RULE = 'Fade only returns beyond k sigma; sit at the current price otherwise.'
SOURCE = "megacellar '164 Trading Strategy Browser' (megacellar-browser.onrender.com), 'ExtremeReturnBounce'"
FAMILY = 'reversion'
DRAFT = False
ANCHOR = False
CLASSES = None
...
What the source published
    If the most recent return is statistically extreme (beyond `k` standard deviations
    of historical returns), it predicts a partial bounce-back (reversion by
    `bounce_pct`) from that extreme move.
...
GRID = (
    {'days': 21.0, 'k': 2.0, 'bounce_pct': 0.5, 'rebal': 7.0},
    {'days': 7.0, 'k': 2.0, 'bounce_pct': 0.5, 'rebal': 7.0},
    {'days': 14.0, 'k': 2.0, 'bounce_pct': 0.5, 'rebal': 7.0},
    {'days': 28.0, 'k': 2.0, 'bounce_pct': 0.5, 'rebal': 7.0},
    {'days': 30.0, 'k': 2.0, 'bounce_pct': 0.5, 'rebal': 30.0},
    {'days': 60.0, 'k': 2.0, 'bounce_pct': 0.5, 'rebal': 30.0},
    {'days': 90.0, 'k': 2.0, 'bounce_pct': 0.5, 'rebal': 30.0},
    {'days': 180.0, 'k': 2.0, 'bounce_pct': 0.5, 'rebal': 30.0},
)


def position(df, close, bpy, days=21.0, k=2.0, bounce_pct=0.5, rebal=7.0):
    """Fade only returns beyond k sigma; sit at the current price otherwise."""
    w = win(bpy, days)
    r = rets(close)
    sd = rstd(r, w)
    pred = np.where(np.abs(r) > k * sd, close * (1.0 - bounce_pct * np.sign(r)), close)
