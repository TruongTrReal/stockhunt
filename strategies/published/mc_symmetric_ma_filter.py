"""A centred moving average, which on a trailing window is the plain SMA."""
...
RULE = 'A centred moving average, which on a trailing window is the plain SMA.'
SOURCE = "megacellar '164 Trading Strategy Browser' (megacellar-browser.onrender.com), 'SymmetricMAFilter'"
FAMILY = 'reversion'
DRAFT = False
ANCHOR = False
CLASSES = None
NOTE = 'RE-IMPLEMENTED FROM A DESCRIPTION, not from source code - the site publishes a one-paragraph account of each rule and no code, so this is a plausible rewrite and not a verified conversion of the original. It is also scored against a different benchmark: the source compares a book that rebalances weekly or monthly against one buy-and-hold number that never rebalances at all, which is the unmatched-schedule failure the root CLAUDE.md lists first. Its published result is not comparable to anything on this sheet. The source centres the average on a point in the past. On causal data that is exactly a trailing SMA of the same length, so this reduces to `close below its own moving average` - the oldest mean-reversion rule there is.'

LOGIC = """
What the source published
    Calculates a centered Simple Moving Average (SMA) by taking an average of prices in
    a window symmetric around a point slightly in the past (window center at `len -
    half_window - 1`). This centered MA is the prediction.
...
def position(df, close, bpy, days=14.0, rebal=7.0):
    """A centred moving average, which on a trailing window is the plain SMA."""
    w = win(bpy, days)
    pred = rmean(close, 2 * max(1, w // 2) + 1)
