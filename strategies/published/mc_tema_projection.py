"""Take the triple exponential moving average as the forecast."""
...
RULE = 'Take the triple exponential moving average as the forecast.'
SOURCE = "megacellar '164 Trading Strategy Browser' (megacellar-browser.onrender.com), 'TEMAProjection'"
FAMILY = 'trend'
DRAFT = False
ANCHOR = False
CLASSES = None
...
What the source published
    Calculates the Triple Exponential Moving Average (TEMA) of prices for a given
    period. The predicted price is the latest TEMA value.
...
def position(df, close, bpy, days=7.0, rebal=7.0):
    """Take the triple exponential moving average as the forecast."""
    w = win(bpy, days)
    pred = talib.TEMA(np.ascontiguousarray(close), timeperiod=max(2, w))
