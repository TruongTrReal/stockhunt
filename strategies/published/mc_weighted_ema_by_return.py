"""Average recent prices with each one weighted by the size of the move that made it."""
...
RULE = 'Average recent prices with each one weighted by the size of the move that made it.'
SOURCE = "megacellar '164 Trading Strategy Browser' (megacellar-browser.onrender.com), 'WeightedEMAByReturn'"
FAMILY = 'reversion'
DRAFT = False
ANCHOR = False
CLASSES = None
...
What the source published
    Calculates a weighted average of recent prices (excluding the first price in
    window). The weight for each price point is its preceding return's absolute
    magnitude (plus a small epsilon).
...
def position(df, close, bpy, days=90.0, rebal=30.0):
    """Average recent prices with each one weighted by the size of the move that made it."""
    w = win(bpy, days)
    wt = np.abs(rets(close)) + 1e-8
    pm, wm = roll(close, w), roll(wt, w)
    bad = nan_mask(pm)
    num = np.nansum(np.nan_to_num(pm, nan=0.0) * np.nan_to_num(wm, nan=0.0), axis=1)
    den = np.nansum(np.nan_to_num(wm, nan=0.0), axis=1)
