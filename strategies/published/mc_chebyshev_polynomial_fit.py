"""Fit a polynomial to the window and extrapolate it one bar."""

from __future__ import annotations

import numpy as np

from strategies._forecast import (expose, poly_extrap, win)

RULE = 'Fit a polynomial to the window and extrapolate it one bar.'
SOURCE = "megacellar '164 Trading Strategy Browser' (megacellar-browser.onrender.com), 'ChebyshevPolynomialFit'"
FAMILY = 'trend'
DRAFT = False
ANCHOR = False
CLASSES = None
NOTE = 'RE-IMPLEMENTED FROM A DESCRIPTION, not from source code - the site publishes a one-paragraph account of each rule and no code, so this is a plausible rewrite and not a verified conversion of the original. It is also scored against a different benchmark: the source compares a book that rebalances weekly or monthly against one buy-and-hold number that never rebalances at all, which is the unmatched-schedule failure the root CLAUDE.md lists first. Its published result is not comparable to anything on this sheet. A Chebyshev least-squares fit on a uniform grid spans the same space as an ordinary polynomial fit of equal degree, so `poly_extrap` gives the identical surface with one pseudo-inverse instead of a fit per window.'

LOGIC = """
What the source published
    Fits a Chebyshev polynomial of a specified degree to recent prices and extrapolates
    this polynomial one step forward to predict the next price.
...
GRID = (
    {'days': 60.0, 'degree': 3.0, 'rebal': 30.0},
    {'days': 7.0, 'degree': 3.0, 'rebal': 7.0},
    {'days': 14.0, 'degree': 3.0, 'rebal': 7.0},
    {'days': 21.0, 'degree': 3.0, 'rebal': 7.0},
    {'days': 28.0, 'degree': 3.0, 'rebal': 7.0},
    {'days': 30.0, 'degree': 3.0, 'rebal': 30.0},
    {'days': 90.0, 'degree': 3.0, 'rebal': 30.0},
    {'days': 180.0, 'degree': 3.0, 'rebal': 30.0},
)


def position(df, close, bpy, days=60.0, degree=3.0, rebal=30.0):
    """Fit a polynomial to the window and extrapolate it one bar."""
    w = win(bpy, days)
    pred = poly_extrap(close, w, int(degree), ahead=1.0)
