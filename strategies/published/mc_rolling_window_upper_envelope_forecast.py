"""In an uptrend predict the upper envelope of the SMA; otherwise predict the SMA."""

from __future__ import annotations

import numpy as np

from strategies._forecast import (expose, rmean, shift, win)

RULE = 'In an uptrend predict the upper envelope of the SMA; otherwise predict the SMA.'
SOURCE = "megacellar '164 Trading Strategy Browser' (megacellar-browser.onrender.com), 'RollingWindowUpperEnvelopeForecast'"
FAMILY = 'trend'
DRAFT = False
ANCHOR = False
CLASSES = None
...
What the source published
    If an uptrend is detected (current price > price `trend_lookback` periods ago), it
    predicts the upper boundary of a percentage-based envelope around the SMA.
    Otherwise, it predicts the SMA.
...
GRID = (
    {'days': 14.0, 'trend_lookback': 20.0, 'envelope_pct': 0.05, 'rebal': 7.0},
    {'days': 7.0, 'trend_lookback': 20.0, 'envelope_pct': 0.05, 'rebal': 7.0},
    {'days': 21.0, 'trend_lookback': 20.0, 'envelope_pct': 0.05, 'rebal': 7.0},
    {'days': 28.0, 'trend_lookback': 20.0, 'envelope_pct': 0.05, 'rebal': 7.0},
    {'days': 30.0, 'trend_lookback': 20.0, 'envelope_pct': 0.05, 'rebal': 30.0},
    {'days': 60.0, 'trend_lookback': 20.0, 'envelope_pct': 0.05, 'rebal': 30.0},
    {'days': 90.0, 'trend_lookback': 20.0, 'envelope_pct': 0.05, 'rebal': 30.0},
    {'days': 180.0, 'trend_lookback': 20.0, 'envelope_pct': 0.05, 'rebal': 30.0},
)


def position(df, close, bpy, days=14.0, trend_lookback=20.0, envelope_pct=0.05, rebal=7.0):
    """In an uptrend predict the upper envelope of the SMA; otherwise predict the SMA."""
    w = win(bpy, days)
    lb = max(2, win(bpy, trend_lookback))
    sma = rmean(close, w)
    up = close > shift(close, lb)
    pred = np.where(up, sma * (1.0 + envelope_pct), sma)
