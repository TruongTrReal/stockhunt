"""Fade the k-th standardised moment of recent returns."""

from __future__ import annotations

import numpy as np

from strategies._forecast import (expose, rets, rmean, rstd, safe_div, win)

RULE = 'Fade the k-th standardised moment of recent returns.'
SOURCE = "megacellar '164 Trading Strategy Browser' (megacellar-browser.onrender.com), 'KthOrderMoment'"
FAMILY = 'reversion'
DRAFT = False
ANCHOR = False
CLASSES = None
...
What the source published
    Calculates the k-th order moment of recent returns. For k=3 (skewness proxy), it
    predicts `current * (1 - moment * 0.1)`. For other k, it predicts based on `current
    * (1 + sign(moment) * 0.05)`.
...
GRID = (
    {'days': 7.0, 'k': 3.0, 'rebal': 7.0},
    {'days': 14.0, 'k': 3.0, 'rebal': 7.0},
    {'days': 21.0, 'k': 3.0, 'rebal': 7.0},
    {'days': 28.0, 'k': 3.0, 'rebal': 7.0},
    {'days': 30.0, 'k': 3.0, 'rebal': 30.0},
    {'days': 60.0, 'k': 3.0, 'rebal': 30.0},
    {'days': 90.0, 'k': 3.0, 'rebal': 30.0},
    {'days': 180.0, 'k': 3.0, 'rebal': 30.0},
)


def position(df, close, bpy, days=7.0, k=3.0, rebal=7.0):
    """Fade the k-th standardised moment of recent returns."""
    w = win(bpy, days)
    r = rets(close)
    mu, sd = rmean(r, w), rstd(r, w)
    z = safe_div(r - mu, sd, fill=np.nan)
    mom = rmean(z ** int(k), w)
    pred = (close * (1.0 - mom * 0.1) if int(k) == 3
            else close * (1.0 + np.sign(mom) * 0.05))
